...
internal readonly record struct CspiceFrameTransform(double M00,double M01,double M02,double M10,double M11,double M12,double M20,double M21,double M22);
internal sealed class CspiceSession : IDisposable
{
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)] delegate int FrameDelegate(IntPtr fromFrame,IntPtr toFrame,double et,out CspiceMatrix3 transform);
    ...
    IntPtr _library; readonly LoadDelegate _load; readonly ClearDelegate _clear; readonly QueryDelegate _query; readonly FrameDelegate _frame; readonly TextDelegate _version; readonly ErrorTextDelegate _error; readonly FailedDelegate _failed; readonly ResetDelegate _reset; bool _ready,_disposed;
    CspiceSession(IntPtr library){_library=library;_load=Bind<LoadDelegate>("NcspLoadKernel");_clear=Bind<ClearDelegate>("NcspClearKernels");_query=Bind<QueryDelegate>("NcspQueryGeometricState");_frame=Bind<FrameDelegate>("NcspQueryFrameTransform");_version=Bind<TextDelegate>("NcspGetToolkitVersion");_error=Bind<ErrorTextDelegate>("NcspGetError");_failed=Bind<FailedDelegate>("NcspHasFailure");_reset=Bind<ResetDelegate>("NcspResetError");}
    ...
    internal bool TryQueryFrame(string fromFrame,string toFrame,double et,out CspiceFrameTransform transform,out CspiceDiagnostic diagnostic)
    {
        transform=default;diagnostic=default;if(_disposed||!_ready||string.IsNullOrWhiteSpace(fromFrame)||string.IsNullOrWhiteSpace(toFrame)||!double.IsFinite(et)){diagnostic=new(_disposed?CspiceSessionStatus.AlreadyDisposed:CspiceSessionStatus.NotReady,"frame-query","","not ready or invalid frame query");return false;}
        var from=Marshal.StringToCoTaskMemUTF8(fromFrame);var to=Marshal.StringToCoTaskMemUTF8(toFrame);try{if(_frame(from,to,et,out var value)==0||_failed()!=0){diagnostic=CaptureFailure("frame-query");return false;}transform=new(value.M00,value.M01,value.M02,value.M10,value.M11,value.M12,value.M20,value.M21,value.M22);return true;}finally{Marshal.FreeCoTaskMem(from);Marshal.FreeCoTaskMem(to);}
    }
