internal static class GeneralizedTransport
{
    ...
    {
        witness=default;
        if(!old.Valid||!next.Valid||old.Features!=next.Features||D3.Dot(old.Normal,next.Normal)<=0||!Feasible(old,cache))return false;
        ...
        return double.IsFinite(screen)&&screen<=1e-12&&pe<=1e-12&&le<=1e-12&&Feasible(next,result);
    }
}
