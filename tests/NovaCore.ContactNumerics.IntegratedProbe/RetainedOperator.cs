...
internal sealed class RetainedOperator
{
        ...
        ReadOnlySpan<double> nativeCache,out Proposal proposal,out Diagnostics proof)
    {
        ...
        if(!GeneralizedTransport.Feasible(current,work))return OperatorStatus.LoadRefusal;
        ...
        if(!GeneralizedTransport.Feasible(current,work))return OperatorStatus.DRefusal;
        int nc=0,tc=0,wc=0;
