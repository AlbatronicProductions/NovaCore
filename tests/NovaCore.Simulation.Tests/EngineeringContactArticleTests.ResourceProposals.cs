...
{
    internal static void ResourceProposalNonActuation()
    {
        foreach (var (tilted, moving) in new[] { (false,false), (true,false), (true,true) })
        {
            ...
            // Explicit zero-resource adapter: all existing article mass stays structural, with no added fuel.
            f.Engine.State.Spacecraft.TryGetTranslation(Craft,out _,out var articleMass);
            f.Engine.State.Spacecraft.TryGetRigidBody(Craft,out var articleAngular);
            Check(PropellantDefinition.TryCreate(new(1,1,1,1,1,1,1,PropellantMassLaw.CentralPointReservoirV1,
                articleMass.MassKilograms,articleAngular.PrincipalInertia),0,out var resourceDefinition)==PropellantPreparationStatus.Ready,"zero-resource definition");
            Check(f.Engine.BindFinitePropellant(authority!,resourceDefinition,out var resource)==PropellantPreparationStatus.Ready,"cold resource binding");
            RunResourceProposals(f, commands!, authority!, resource!, expected);
            Console.WriteLine($"RESOURCE_ARTICLE PASS tilted={tilted} moving={moving} endpoint_bits=1200/1200 original_geometry_gates=PASS same_world=PASS no_force_applied=PASS");
        }
    }
    private static void RunResourceProposals(Fixture f, SpacecraftCommandAuthority commands, EnginePreparationAuthority authority, PropellantResourceAuthority resource,
        NovaCore.Simulation.Spacecraft.Translation.SpacecraftMotion[] expected)
    {
        ...
        for(var step=1;step<=1200;step++)
        {
            ...
            Check(f.Engine.ObserveFinitePropellant(resource,out var resourceBefore,out _)==PropellantPreparationStatus.Ready,"resource source before");
            Check(f.Engine.PrepareFinitePropellant(resource,token,target,out var resourceToken)==PropellantPreparationStatus.Prepared &&
                f.Engine.PreviewFinitePropellant(resource,resourceToken,out var resourcePreview)==PropellantPreparationStatus.Preview &&
                resourcePreview.Classification==PropellantClassification.NoFeed && resourcePreview.Engine.ProposedLatch==ProposedEngineLatch.Enabled &&
                resourcePreview.ConsumedUnits.IsZero && resourcePreview.SuccessorUnits.IsZero,"nonzero engine demand but empty conditional feed");
            Check(f.Engine.RetireFinitePropellant(resource,resourceToken)==PropellantPreparationStatus.Retired &&
                f.Engine.ObserveFinitePropellant(resource,out var resourceAfter,out _)==PropellantPreparationStatus.Ready && resourceBefore==resourceAfter,
                "resource preparation and retirement leave canonical amount/revision unchanged");
