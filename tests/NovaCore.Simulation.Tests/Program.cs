...
var tests = new (string Name, Action Test)[]
{
    ...
    ("SolAnalytical dataset", SolAnalyticalDatasetTests),
...
static void CelestialBodyOrientationTests()
{
    ...
    var anchorLatitude=.4d;var anchorLongitude=-1.2d;var anchorLatitudeCosine=Math.Cos(anchorLatitude);var anchorDirection=new Double3(anchorLatitudeCosine*Math.Cos(anchorLongitude),Math.Sin(anchorLatitude),anchorLatitudeCosine*Math.Sin(anchorLongitude));var local=anchorDirection*(6_371_008.8d+250d);Check(CelestialBodyFixedFrameEvaluator.TryTransformBodyFixedPosition(SolarSystemBodyIds.Earth,SimulationInstant.Zero,local,new Double3(1e11,2e11,-3e11),out var anchor0)&&CelestialBodyFixedFrameEvaluator.TryTransformBodyFixedPosition(SolarSystemBodyIds.Earth,SimulationInstant.FromWholeSeconds(3600),local,new Double3(1e11,2e11,-3e11),out var anchor1)&&anchor0!=anchor1,"body-fixed point rotates in inertial space while retaining exact local identity");
...
static void CelestialSystemDefinitionTests()
{
    ...
    // Literal names preserve the pre-migration semantic hash; no identity is inferred from bindings.
    CelestialBodyCatalogEntry[] bodies = [new CelestialBodyCatalogEntry(new(new(30), "Legacy-30", CelestialBodyClassification.Other, new(10), default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default)), new CelestialBodyCatalogEntry(new(new(20), "Legacy-20", CelestialBodyClassification.Other, new(10), default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default)), new CelestialBodyCatalogEntry(new(new(10), "Legacy-10", CelestialBodyClassification.Other, null, default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default))];
    var nodes = new[] { new CelestialHierarchyNode(new CelestialBodyId(30), new(CelestialTrajectoryModel.AnalyticalKepler, new(1), 0)), new CelestialHierarchyNode(new CelestialBodyId(20), new(CelestialTrajectoryModel.CircularOrbit, new(3), 0)), new CelestialHierarchyNode(new CelestialBodyId(10), new(CelestialTrajectoryModel.FixedBody, new(2), 0)) };
    CelestialEphemerisSource[] sources = [new(new(2), CelestialTrajectoryModel.FixedBody, Metadata() with { Source = new(2) }), new(new(3), CelestialTrajectoryModel.CircularOrbit, Metadata() with { Source = new(3) }), new(new(1), CelestialTrajectoryModel.AnalyticalKepler, Metadata())];
    FixedBodyEphemerisPayload[] fixedBodies = [FixedBodyEphemerisPayload.Identity];
    CircularOrbitEphemerisPayload[] circular = [new(0, 1d, 0d, DoubleQuaternion.Identity, 1d)];
    TwoBodyTrajectory[] analytical = [new(new(10), SimulationInstant.Zero, new(new Double3(1d, 0d, 0d), new Double3(0d, 1d, 0d)), TwoBodyPropagationModel.CartesianTwoBodyV1)];
    Check(CelestialSystemDefinition.TryCreate(new(77), bodies, nodes, Mapping(), Metadata(), sources, fixedBodies, circular, analytical, out var ordered, out var validation) && ordered is not null && validation.Succeeded && ordered.GetNodeInTraversalOrder(0).Id.Value == 10 && ordered.GetNodeInTraversalOrder(1).Id.Value == 20 && ordered.GetNodeInTraversalOrder(2).Id.Value == 30, "traversal order is root-first then stable body ID");
    var hash = CelestialSystemDefinitionHash.Compute(ordered!); Check(hash == 0xAB869A12B4D3734AUL, "explicit authoring preserves pre-migration definition identity");
    Check(CelestialSystemDefinition.TryCreate(new(77), bodies, nodes, Mapping(), Metadata(), sources, fixedBodies, circular, analytical, out var repeated, out _) && repeated is not null && CelestialSystemDefinitionHash.Compute(repeated) == hash, "repeated system construction hash");
    nodes[0] = new CelestialHierarchyNode(new CelestialBodyId(30), new(CelestialTrajectoryModel.FixedBody, new(2), 0));
    bodies[0] = bodies[0] with { Identity = bodies[0].Identity with { ParentBody = null } };
    sources[0] = sources[0] with { Metadata = Metadata() with { Source = new(2), Version = new(9) } };
    fixedBodies[0] = FixedBodyEphemerisPayload.Identity with { Position = new Double3(1d, 0d, 0d) };
    circular[0] = circular[0] with { InitialPhaseRadians = .25d };
    analytical[0] = analytical[0] with { CentralBody = new(99) };
    Check(ordered!.GetBody(0).Identity.ParentBody == new CelestialBodyId(10) && ordered.GetNode(0).TrajectoryModel == CelestialTrajectoryModel.AnalyticalKepler && CelestialSystemDefinitionHash.Compute(ordered) == hash, "system definition copies catalog, bindings, sources and payloads");

    CheckSystem([new CelestialBodyCatalogEntry(new(new(1), "Legacy-1", CelestialBodyClassification.Other, null, default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default)), new CelestialBodyCatalogEntry(new(new(1), "Legacy-1", CelestialBodyClassification.Other, new(2), default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default))], [new CelestialHierarchyNode(new CelestialBodyId(1), new(CelestialTrajectoryModel.FixedBody, new(2), 0)), new CelestialHierarchyNode(new CelestialBodyId(1), new(CelestialTrajectoryModel.AnalyticalKepler, new(1), 0))], CelestialSystemValidationStatus.DuplicateBodyId, "duplicate ID rejection");
    CheckSystem([new CelestialBodyCatalogEntry(new(new(1), "Legacy-1", CelestialBodyClassification.Other, null, default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default)), new CelestialBodyCatalogEntry(new(new(2), "Legacy-2", CelestialBodyClassification.Other, new(3), default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default)), new CelestialBodyCatalogEntry(new(new(3), "Legacy-3", CelestialBodyClassification.Other, new(2), default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default))], [new CelestialHierarchyNode(new CelestialBodyId(1), new(CelestialTrajectoryModel.FixedBody, new(2), 0)), new CelestialHierarchyNode(new CelestialBodyId(2), new(CelestialTrajectoryModel.AnalyticalKepler, new(1), 0)), new CelestialHierarchyNode(new CelestialBodyId(3), new(CelestialTrajectoryModel.AnalyticalKepler, new(1), 0))], CelestialSystemValidationStatus.ParentCycle, "cycle rejection");
    CheckSystem([new CelestialBodyCatalogEntry(new(new(1), "Legacy-1", CelestialBodyClassification.Other, null, default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default)), new CelestialBodyCatalogEntry(new(new(2), "Legacy-2", CelestialBodyClassification.Other, new(99), default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default))], [new CelestialHierarchyNode(new CelestialBodyId(1), new(CelestialTrajectoryModel.FixedBody, new(2), 0)), new CelestialHierarchyNode(new CelestialBodyId(2), new(CelestialTrajectoryModel.AnalyticalKepler, new(1), 0))], CelestialSystemValidationStatus.MissingCatalogParent, "missing parent rejection");
    CheckSystem([new CelestialBodyCatalogEntry(new(new(1), "Legacy-1", CelestialBodyClassification.Other, null, default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default)), new CelestialBodyCatalogEntry(new(new(2), "Legacy-2", CelestialBodyClassification.Other, null, default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default))], [new CelestialHierarchyNode(new CelestialBodyId(1), new(CelestialTrajectoryModel.FixedBody, new(2), 0)), new CelestialHierarchyNode(new CelestialBodyId(2), new(CelestialTrajectoryModel.FixedBody, new(2), 0))], CelestialSystemValidationStatus.MultipleRoots, "multiple roots rejection");
    ...
    static void CheckSystem(CelestialBodyCatalogEntry[] catalog, CelestialHierarchyNode[] candidates, CelestialSystemValidationStatus expected, string message)
    {
        // Explicit payload/source inputs; these malformed cases reject before publication.
        CelestialEphemerisSource[] sources = [new(new(2), CelestialTrajectoryModel.FixedBody, Metadata() with { Source = new(2) }), new(new(1), CelestialTrajectoryModel.AnalyticalKepler, Metadata())];
        Check(!CelestialSystemDefinition.TryCreate(new(99), catalog, candidates, Mapping(), Metadata(), sources, [FixedBodyEphemerisPayload.Identity], [], [new(new(1), SimulationInstant.Zero, new(new Double3(1d, 0d, 0d), new Double3(0d, 1d, 0d)), TwoBodyPropagationModel.CartesianTwoBodyV1)], out var definition, out var result) && definition is null && result.Status == expected, message);
    }
}
...
{
    var constructionBefore = GC.GetAllocatedBytesForCurrentThread(); var constructed = SolAnalyticalDefinition.CreateForTest(); var constructionAllocated = GC.GetAllocatedBytesForCurrentThread() - constructionBefore;
    var system = SolAnalyticalDefinition.Instance;
    var periodicSource=new[]{new AnalyticalKeplerPeriodicTerm(1d,2d,3d,4d,5d)};var periodicCopy=new AnalyticalKeplerPeriodicCorrection(periodicSource);periodicSource[0]=default;Check(periodicCopy.Count==1&&periodicCopy.GetTerm(0)==new AnalyticalKeplerPeriodicTerm(1d,2d,3d,4d,5d)&&!new AnalyticalKeplerPeriodicCorrection([new AnalyticalKeplerPeriodicTerm(double.NaN,0d,0d,0d,0d)]).IsValid,"periodic correction copies caller data and validates fixed-width terms");var budgetRejected=false;try{_ = new AnalyticalKeplerPeriodicCorrection(new AnalyticalKeplerPeriodicTerm[9]);}catch(ArgumentOutOfRangeException){budgetRejected=true;}Check(budgetRejected,"periodic correction enforces eight-component hard budget");
    Check(system.Id == SolAnalyticalDefinition.Id && SolAnalyticalDefinition.VersionName == "SolCompact-DE440Validated-v3" && system.RootBody == SolarSystemBodyIds.Sun && system.Count == 10, "stable versioned compact Solar identity and Sun root");
    var required = new[] { SolarSystemBodyIds.Sun, SolarSystemBodyIds.Mercury, SolarSystemBodyIds.Venus, SolarSystemBodyIds.Earth, SolarSystemBodyIds.Moon, SolarSystemBodyIds.Mars, SolarSystemBodyIds.Jupiter, SolarSystemBodyIds.Saturn, SolarSystemBodyIds.Uranus, SolarSystemBodyIds.Neptune };
    for (var index = 0; index < required.Length; index++) Check(system.TryGetBody(required[index], out var body) && body.Id == required[index] && body.PhysicalProperties.IsValid, "required stable Solar body");
    Check(system.TryGetBody(SolarSystemBodyIds.Moon, out var moonBody) && moonBody.Identity.ParentBody == SolarSystemBodyIds.Earth && system.TryGetNode(SolarSystemBodyIds.Moon, out var moonNode) && moonNode.TrajectoryModel == CelestialTrajectoryModel.AnalyticalKepler, "Moon is Earth-relative analytical body");
    for (var index = 0; index < system.Count; index++) { var node = system.GetNodeInTraversalOrder(index); Check(index == 0 ? node.Id == SolarSystemBodyIds.Sun && node.TrajectoryModel == CelestialTrajectoryModel.FixedBody : node.TrajectoryModel == CelestialTrajectoryModel.AnalyticalKepler, "generic SolAnalytical bindings"); }
    var values = new ReferenceFrameEvaluation[system.Count]; var roots = new FrameTransform[system.Count]; var staging = new ReferenceFrameEvaluation[system.Count]; var rootStaging = new FrameTransform[system.Count];
    Check(CelestialSystemEvaluator.TryEvaluateSystem(system, SimulationInstant.Zero, values, roots, staging, rootStaging).Succeeded, "epoch SolAnalytical evaluation");
    var sun = Root(roots, values, SolarSystemBodyIds.Sun); var earth = Root(roots, values, SolarSystemBodyIds.Earth); var moon = Root(roots, values, SolarSystemBodyIds.Moon); var mercury = Root(roots, values, SolarSystemBodyIds.Mercury); var mars = Root(roots, values, SolarSystemBodyIds.Mars); var jupiter = Root(roots, values, SolarSystemBodyIds.Jupiter); var neptune = Root(roots, values, SolarSystemBodyIds.Neptune);
    double maximumPositionError = 0d, maximumVelocityError = 0d, maximumInvariantError = 0d; ulong sourceHash = 14695981039346656037UL, conversionHash = 14695981039346656037UL, invariantHash = 14695981039346656037UL;
    for (var index = 0; index < SolAnalyticalDefinition.ElementCount; index++)
    {
        var element = SolAnalyticalDefinition.GetElement(index); var expected = ConvertIndependent(element); Check(system.TryGetAnalyticalKepler(index, out var stored) && stored.CentralBody == element.Parent, "authored element trajectory binding");
        var positionError = Magnitude(expected.Position - stored.StateAtEpoch.Position); var velocityError = Magnitude(expected.Velocity - stored.StateAtEpoch.Velocity); maximumPositionError = Math.Max(maximumPositionError, positionError); maximumVelocityError = Math.Max(maximumVelocityError, velocityError);
        Check(positionError <= 1e-5d && velocityError <= 1e-10d, "independent source-to-Cartesian fidelity");
        var r = stored.StateAtEpoch.Position; var v = stored.StateAtEpoch.Velocity; var radius = Magnitude(r); var h = Double3.Cross(r, v); var energy = v.LengthSquared * .5d - element.CentralGravitationalParameter / radius; var semiMajor = -element.CentralGravitationalParameter / (2d * energy); var eccentricity = Magnitude(Double3.Cross(v, h) / element.CentralGravitationalParameter - r / radius); var inclination = Math.Acos(Math.Clamp(h.Z / Magnitude(h), -1d, 1d));
        var invariantError = Math.Max(Relative(semiMajor, element.SemiMajorAxisAu * SolAnalyticalDefinition.AstronomicalUnitMetres), Math.Max(Math.Abs(eccentricity - element.Eccentricity), Math.Abs(inclination - Math.Abs(Degrees(element.InclinationDegrees))))); maximumInvariantError = Math.Max(maximumInvariantError, invariantError);
        var inclinationTolerance = Math.Abs(element.InclinationDegrees) < 1e-3d ? 1e-8d : 2e-12d; Check(Relative(semiMajor, element.SemiMajorAxisAu * SolAnalyticalDefinition.AstronomicalUnitMetres) < 2e-12d && Math.Abs(eccentricity - element.Eccentricity) < 2e-12d && Math.Abs(inclination - Math.Abs(Degrees(element.InclinationDegrees))) < inclinationTolerance && radius >= semiMajor * (1d - eccentricity) && radius <= semiMajor * (1d + eccentricity), $"orbital invariant fidelity {element.Body.Value}: a={Relative(semiMajor,element.SemiMajorAxisAu*SolAnalyticalDefinition.AstronomicalUnitMetres):E3}; e={Math.Abs(eccentricity-element.Eccentricity):E3}; i={Math.Abs(inclination-Math.Abs(Degrees(element.InclinationDegrees))):E3}");
        var runtime = Local(values, element.Body);
        if (element.Body == SolarSystemBodyIds.Moon)
        {
            Check(system.TryGetAnalyticalCorrection(index,out var lunarCorrection)&&lunarCorrection==SolAnalyticalDefinition.LunarCorrection&&!lunarCorrection.IsIdentity,"Moon binds the authored compact secular correction");
            Check(system.TryGetAnalyticalPeriodicCorrection(index,out var lunarPeriodic)&&lunarPeriodic==SolAnalyticalDefinition.LunarPeriodicCorrection&&!lunarPeriodic.IsIdentity&&lunarPeriodic.Count==7,"Moon binds the bounded seven-frequency periodic correction");
            Check(AnalyticalKeplerSecularCorrectionEvaluator.TryApply(stored.StateAtEpoch,stored.StateAtEpoch,stored.Epoch,stored.Epoch,lunarCorrection,lunarPeriodic,out var correctedEpoch)&&runtime==correctedEpoch&&Magnitude(runtime.Position-stored.StateAtEpoch.Position)<=1e-5d,"corrected Moon preserves its grounded ET0 position and deterministic direct velocity");
        }
        else Check(system.TryGetAnalyticalCorrection(index,out var identityCorrection)&&identityCorrection.IsIdentity&&system.TryGetAnalyticalPeriodicCorrection(index,out var identityPeriodic)&&identityPeriodic.IsIdentity&&Magnitude(runtime.Position-stored.StateAtEpoch.Position)<=1e-5d&&Magnitude(runtime.Velocity-stored.StateAtEpoch.Velocity)<=1e-10d,"uncorrected exact epoch runtime state fidelity");
        sourceHash = HashElement(sourceHash, element); conversionHash = MixDouble3(MixDouble3(conversionHash, stored.StateAtEpoch.Position), stored.StateAtEpoch.Velocity); invariantHash = Mix(invariantHash, (ulong)BitConverter.DoubleToInt64Bits(semiMajor));
    }
    Check(sun.LengthSquared == 0d && Approx(Magnitude(earth - sun), SolAnalyticalDefinition.AstronomicalUnitMetres, .03d) && Approx(Magnitude(moon - earth), 384_400_000d, .08d), "secondary human-readable Earth and Moon scale smoke check");
    Check(Magnitude(mercury) > .25d * SolAnalyticalDefinition.AstronomicalUnitMetres && Magnitude(mars) > SolAnalyticalDefinition.AstronomicalUnitMetres && Magnitude(jupiter) > 4d * SolAnalyticalDefinition.AstronomicalUnitMetres && Magnitude(neptune) > 25d * SolAnalyticalDefinition.AstronomicalUnitMetres, "secondary planetary scale smoke check");
    var earthVelocity = Magnitude(Velocity(values, SolarSystemBodyIds.Earth)); var moonVelocity = Magnitude(Velocity(values, SolarSystemBodyIds.Moon)); Check(earthVelocity > 25_000d && earthVelocity < 35_000d && moonVelocity > 900d && moonVelocity < 1_200d, "representative orbital velocities");
    Check(system.TryGetPhysicalProperties(SolarSystemBodyIds.Sun,out var sunProperties),"compact Solar Sun constants");var sunMu=sunProperties.GravitationalParameter;
    var directTime = SimulationInstant.FromWholeSeconds(86_400); var directTrajectory = system.GetAnalyticalKepler(2); var direct = UniversalVariableTwoBodyPropagator.TryEvaluate(directTrajectory.StateAtEpoch, directTrajectory.Epoch, directTime, sunMu); Check(direct.Succeeded && CelestialSystemEvaluator.TryEvaluateSystem(system, directTime, values, roots, staging, rootStaging).Succeeded, "direct/generic comparison evaluation"); var generic = Local(values, SolarSystemBodyIds.Earth); var directPositionError = Magnitude(direct.State.Position - generic.Position); var directVelocityError = Magnitude(direct.State.Velocity - generic.Velocity); Check(directPositionError <= 1e-5d && directVelocityError <= 1e-10d, "direct and generic evaluator consistency");
    var segmentedTime = SimulationInstant.FromWholeSeconds((long)(100d * SolAnalyticalDefinition.JulianYearSeconds)); var segmented = Segment(directTrajectory.StateAtEpoch, SimulationInstant.Zero, segmentedTime, sunMu, out var segments); Check(segments == 2 && CelestialSystemEvaluator.TryEvaluateSystem(system, segmentedTime, values, roots, staging, rootStaging).Succeeded, "deterministic bounded segment policy"); var segmentedGeneric = Local(values, SolarSystemBodyIds.Earth); var segmentedPositionError = Magnitude(segmented.Position - segmentedGeneric.Position); var segmentedVelocityError = Magnitude(segmented.Velocity - segmentedGeneric.Velocity); Check(segmentedPositionError <= 1e-3d && segmentedVelocityError <= 1e-8d, "segmented composition consistency");
    Check(CelestialSystemEvaluator.TryEvaluateSystem(system, SimulationInstant.Zero, values, roots, staging, rootStaging).Succeeded, "restore epoch before epoch hash"); var epochHash = CelestialSystemEvaluationHash.Compute(values); var definitionHash = CelestialSystemDefinitionHash.Compute(system); var catalogHash = SolCatalogHash(system); var trajectoryHash = SolTrajectoryHash(system); var earthMoonHash = Mix(Mix(Mix(14695981039346656037UL, (ulong)BitConverter.DoubleToInt64Bits((moon - earth).X)), (ulong)BitConverter.DoubleToInt64Bits((moon - earth).Y)), (ulong)BitConverter.DoubleToInt64Bits((moon - earth).Z)); ulong broadHash = 14695981039346656037UL;
    Check(definitionHash==0x493FE8B1E867110FUL&&CelestialSystemDefinitionHash.Compute(SolAnalyticalDefinition.CreateV2ForTest())==0xDC999161489DC66CUL,"versioned v2/v3 definition hashes");
    var times = new[] { SimulationInstant.Zero, SimulationInstant.FromWholeSeconds(-86_400), SimulationInstant.FromWholeSeconds(86_400), SimulationInstant.FromWholeSeconds(-(long)SolAnalyticalDefinition.JulianYearSeconds), SimulationInstant.FromWholeSeconds((long)SolAnalyticalDefinition.JulianYearSeconds), SimulationInstant.FromWholeSeconds(-(long)(100d * SolAnalyticalDefinition.JulianYearSeconds)), SimulationInstant.FromWholeSeconds((long)(100d * SolAnalyticalDefinition.JulianYearSeconds)), SimulationInstant.FromWholeSeconds(-15_778_800_000), SimulationInstant.FromWholeSeconds(15_778_800_000) };
    foreach (var time in times) { var result = CelestialSystemEvaluator.TryEvaluateSystem(system, time, values, roots, staging, rootStaging); Check(result.Succeeded, $"broad finite SolAnalytical evaluation: {time.Ticks} {result.Status}"); broadHash = Mix(broadHash, CelestialSystemEvaluationHash.Compute(values)); }
    Check(CelestialSystemEvaluator.TryEvaluateSystem(system, SimulationInstant.FromWholeSeconds(15_778_800_001), values, roots, staging, rootStaging).Status == CelestialSystemEvaluationStatus.TimeMappingFailure, "outside declared coverage rejected");
    Check(CelestialSystemEvaluator.TryEvaluateSystem(system, SimulationInstant.Zero, values, roots, staging, rootStaging).Succeeded, "restore epoch evaluation"); var earthCenteredSun = Root(roots, values, SolarSystemBodyIds.Sun) - Root(roots, values, SolarSystemBodyIds.Earth); var earthCenteredMoon = Root(roots, values, SolarSystemBodyIds.Moon) - Root(roots, values, SolarSystemBodyIds.Earth); var earthCenteredMars = Root(roots, values, SolarSystemBodyIds.Mars) - Root(roots, values, SolarSystemBodyIds.Earth);
    ...
    static Double3 Root(ReadOnlySpan<FrameTransform> roots, ReadOnlySpan<ReferenceFrameEvaluation> values, CelestialBodyId id) { for (var index = 0; index < values.Length; index++) if (values[index].Frame.Value == (long)id.Value) return roots[index].Translation; throw new Exception("Body evaluation missing."); }
    static Double3 Velocity(ReadOnlySpan<ReferenceFrameEvaluation> values, CelestialBodyId id) { for (var index = 0; index < values.Length; index++) if (values[index].Frame.Value == (long)id.Value) return values[index].Value.OriginVelocityInParent; throw new Exception("Body evaluation missing."); }
    static CartesianState Local(ReadOnlySpan<ReferenceFrameEvaluation> values, CelestialBodyId id) { for (var index = 0; index < values.Length; index++) if (values[index].Frame.Value == (long)id.Value) return new(values[index].Value.LocalToParent.Translation, values[index].Value.OriginVelocityInParent); throw new Exception("Body evaluation missing."); }
    static CartesianState ConvertIndependent(in SolAnalyticalOrbitalElements e) { var a=e.SemiMajorAxisAu*SolAnalyticalDefinition.AstronomicalUnitMetres;var i=Degrees(e.InclinationDegrees);var n=Degrees(e.LongitudeOfAscendingNodeDegrees);var w=Degrees(e.ArgumentOfPeriapsisDegrees);var m=Degrees(e.MeanAnomalyDegrees);var anomaly=m;for(var k=0;k<16;k++)anomaly-=(anomaly-e.Eccentricity*Math.Sin(anomaly)-m)/(1d-e.Eccentricity*Math.Cos(anomaly));var c=Math.Cos(anomaly);var s=Math.Sin(anomaly);var r=a*(1d-e.Eccentricity*c);var x=a*(c-e.Eccentricity);var y=a*Math.Sqrt(1d-e.Eccentricity*e.Eccentricity)*s;var factor=Math.Sqrt(e.CentralGravitationalParameter*a)/r;return new(Rotate(x,y,i,n,w),Rotate(-factor*s,factor*Math.Sqrt(1d-e.Eccentricity*e.Eccentricity)*c,i,n,w)); }
    static Double3 Rotate(double x,double y,double i,double n,double w){var co=Math.Cos(n);var so=Math.Sin(n);var ci=Math.Cos(i);var si=Math.Sin(i);var cw=Math.Cos(w);var sw=Math.Sin(w);return new((cw*co-sw*so*ci)*x+(-sw*co-cw*so*ci)*y,(cw*so+sw*co*ci)*x+(-sw*so+cw*co*ci)*y,sw*si*x+cw*si*y);}
    static CartesianState Segment(CartesianState state,SimulationInstant epoch,SimulationInstant target,double mu,out int count){count=0;var current=state;var at=epoch;var remaining=(target-epoch).Ticks;while(remaining!=0){var step=Math.Min(remaining,UniversalVariableTwoBodyPropagator.MaximumEvaluationTicks);var next=at+new SimulationDuration(step);var result=UniversalVariableTwoBodyPropagator.TryEvaluate(current,at,next,mu);if(!result.Succeeded)throw new Exception("segment evaluation failed");current=result.State;at=next;remaining-=step;count++;}return current;}
    static ulong HashElement(ulong hash,in SolAnalyticalOrbitalElements e)=>Mix(Mix(Mix(hash,e.Body.Value),e.Parent.Value),(ulong)BitConverter.DoubleToInt64Bits(e.SemiMajorAxisAu));
    static double Relative(double left,double right)=>Math.Abs(left-right)/Math.Max(Math.Abs(right),double.Epsilon);
    static double Degrees(double value) => value * Math.PI / 180d;
    static double Magnitude(Double3 value) => Math.Sqrt(value.LengthSquared);
    static bool Approx(double value, double expected, double relative) => Math.Abs(value - expected) <= expected * relative;
}
...
static void CelestialSystemTimeAndProvenanceTests()
{
    ...
    CelestialBodyCatalogEntry[] bodies = [new CelestialBodyCatalogEntry(new(new(1), "Legacy-1", CelestialBodyClassification.Other, null, default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default))];
    var nodes = new[] { new CelestialHierarchyNode(new CelestialBodyId(1), new(CelestialTrajectoryModel.FixedBody, new(2), 0)) };
    var metadata = new CelestialEphemerisMetadata(new(1), new(2), domain, -5, 5, new(3), new(4), new(5, 6), new(7, 8));
    Check(CelestialSystemDefinition.TryCreate(new(400), bodies, nodes, new(SimulationInstant.Zero, new(domain, 0, 1), 1, 1), metadata, [new(new(2), CelestialTrajectoryModel.FixedBody, metadata with { Source = new(2) })], [FixedBodyEphemerisPayload.Identity], [], [], out var system, out var validation) && system is not null && validation.Succeeded, "one system-wide mapping and compatible metadata");
    ...
    Check(!CelestialSystemDefinition.TryCreate(new(401), bodies, nodes, new(SimulationInstant.Zero, new(domain, 0, 1), 1, 1), metadata with { Domain = new(8) }, [new(new(2), CelestialTrajectoryModel.FixedBody, metadata with { Domain = new(8) } with { Source = new(2) })], [FixedBodyEphemerisPayload.Identity], [], [], out _, out validation) && validation.Status == CelestialSystemValidationStatus.MappingMetadataDomainMismatch, "metadata domain compatibility");
    Check(!CelestialSystemDefinition.TryCreate(new(401), bodies, nodes, new(SimulationInstant.Zero, new(domain, 0, 1), 1, 1), metadata with { SupportedStartDomainTicks = 6, SupportedEndDomainTicks = 5 }, [new(new(2), CelestialTrajectoryModel.FixedBody, metadata with { SupportedStartDomainTicks = 6, SupportedEndDomainTicks = 5 } with { Source = new(2) })], [FixedBodyEphemerisPayload.Identity], [], [], out _, out validation) && validation.Status == CelestialSystemValidationStatus.InvalidSupportedInterval, "invalid interval rejection");
...
static void CelestialEphemerisCatalogTests()
{
    ...
    CelestialBodyCatalogEntry[] bodies = [new CelestialBodyCatalogEntry(new(new(1), "Legacy-1", CelestialBodyClassification.Other, null, default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default)), new CelestialBodyCatalogEntry(new(new(2), "Legacy-2", CelestialBodyClassification.Other, new(1), default, default, default), new(1d, 0d, 0d, 0d, 0d, default, default, default))];
    var nodes = new[] { new CelestialHierarchyNode(new CelestialBodyId(1), new(CelestialTrajectoryModel.FixedBody, new(1), 0)), new CelestialHierarchyNode(new CelestialBodyId(2), new(CelestialTrajectoryModel.CircularOrbit, new(2), 0)) };
    Check(CelestialSystemDefinition.TryCreate(new(901), bodies, nodes, mapping, metadata, [fixedSource, circularSource], fixedPayloads, circularPayloads, [], out var system, out var validation) && system is not null && validation.Succeeded, "typed FixedBody and CircularOrbit bindings validate");
    var constructionBefore = GC.GetAllocatedBytesForCurrentThread(); Check(CelestialSystemDefinition.TryCreate(new(901), bodies, nodes, mapping, metadata, [fixedSource, circularSource], fixedPayloads, circularPayloads, [], out var constructionCopy, out _) && constructionCopy is not null, "catalog construction copy"); var constructionAllocated = GC.GetAllocatedBytesForCurrentThread() - constructionBefore;
    ...
    var hash = CelestialSystemDefinitionHash.Compute(system); Check(hash == 0x393A2730EA3A1BF5UL, "explicit typed catalog preserves pre-migration hash");
    Check(CelestialSystemDefinition.TryCreate(new(901), bodies, nodes, mapping, metadata, [fixedSource, circularSource], fixedPayloads, [circularPayloads[0] with { InitialPhaseRadians = .25d }], [], out var changed, out _) && CelestialSystemDefinitionHash.Compute(changed!) != hash, "payload value changes definition hash");
    Check(CelestialSystemDefinition.TryCreate(new(901), bodies, nodes, mapping, metadata, [circularSource, fixedSource], fixedPayloads, circularPayloads, [], out changed, out _) && CelestialSystemDefinitionHash.Compute(changed!) != hash, "source declaration order changes definition hash");
    Check(!CelestialSystemDefinition.TryCreate(new(902), bodies, nodes, mapping, metadata, [fixedSource, fixedSource], fixedPayloads, circularPayloads, [], out _, out validation) && validation.Status == CelestialSystemValidationStatus.DuplicateEphemerisSourceId, "duplicate source rejection");
    Check(!CelestialSystemDefinition.TryCreate(new(902), bodies, [nodes[0], nodes[1] with { Ephemeris = new(CelestialTrajectoryModel.CircularOrbit, new(9), 0) }], mapping, metadata, [fixedSource, circularSource], fixedPayloads, circularPayloads, [], out _, out validation) && validation.Status == CelestialSystemValidationStatus.MissingEphemerisSource, "missing source rejection");
...
static void CelestialSystemEvaluationTests()
{
    ...
    Check(!CelestialSystemDefinition.TryCreate(new(900), unsupportedBodies, unsupportedNodes, CelestialSystemTimeMapping.Identity(new(1)), unsupportedMetadata, [new(new(2), CelestialTrajectoryModel.FixedBody, unsupportedMetadata with { Source = new(2) }), new(new(5), CelestialTrajectoryModel.ReservedNumericalNBody, unsupportedMetadata with { Source = new(5) })], [FixedBodyEphemerisPayload.Identity], [], [], out _, out var unsupportedValidation) && unsupportedValidation.Status == CelestialSystemValidationStatus.UnsupportedReservedTrajectoryModel, "reserved numerical model rejects before publication");
...
static ulong SolCatalogHash(CelestialSystemDefinition system){ulong hash=14695981039346656037;for(var index=0;index<system.BodyCount;index++){var entry=system.GetBody(index);hash=Mix(hash,entry.Id.Value);hash=Mix(hash,(ulong)BitConverter.DoubleToInt64Bits(entry.PhysicalProperties.GravitationalParameter));hash=Mix(hash,(ulong)BitConverter.DoubleToInt64Bits(entry.PhysicalProperties.EquatorialRadius));}return hash;}
static ulong SolTrajectoryHash(CelestialSystemDefinition system){ulong hash=14695981039346656037;for(var index=0;index<system.AnalyticalKeplerCount;index++){var trajectory=system.GetAnalyticalKepler(index);hash=Mix(hash,trajectory.CentralBody.Value);hash=MixDouble3(hash,trajectory.StateAtEpoch.Position);hash=MixDouble3(hash,trajectory.StateAtEpoch.Velocity);}return hash;}
