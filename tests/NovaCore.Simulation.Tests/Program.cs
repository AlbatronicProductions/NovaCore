...
static void SolAnalyticalDatasetTests()
{
    ...
    Check(system.Id == SolAnalyticalDefinition.Id && system.RootBody == SolarSystemBodyIds.Sun && system.Count == 10, "stable SolAnalytical identity and Sun root");
    ...
    for (var index = 0; index < SolAnalyticalDefinition.ElementCount; index++)
    {
        ...
        var runtime = Local(values, element.Body); Check(Magnitude(runtime.Position - stored.StateAtEpoch.Position) <= 1e-5d && Magnitude(runtime.Velocity - stored.StateAtEpoch.Velocity) <= 1e-10d, "exact epoch runtime state fidelity");
