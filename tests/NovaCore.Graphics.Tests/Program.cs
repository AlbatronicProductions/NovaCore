...
{
    var root=new ReferenceFrameId(1);var body=new PlanetRenderProxy(42,new UniversePosition(Double3.Zero,root),10,new Float3(.1f,.2f,.3f),"Generic",true);var before=body;var config=new PlanetaryRepresentationHandoffConfiguration(12,18,.25);
...
static void SolarSystemSceneTest()
{
    ...
    var camera=new CameraState(new FramePosition(root,new Double3(0,0,SolAnalyticalDefinition.AstronomicalUnitMetres*45)),DoubleQuaternion.Identity,sol.Projection,CameraMode.Free);sol.Update(camera);Check(sol.FocusedBlend.Regime==PlanetaryRenderRegime.DistantOnly&&!sol.DetailedComputeRequested,"focused far body remains distant-only");
    Check(bodies.Select(body=>body.Label).Distinct().Count()==10,"presentation labels map uniquely to body identities");Check(sol.OrbitRootSamples.Length==SolarSystemScene.OrbitPathCount*SolarSystemScene.OrbitSampleCount&&sol.OrbitVertices.Length==SolarSystemScene.OrbitVertexCount,"bounded nine-path solar trajectory transport");var frozenOrbit=sol.OrbitRootSamples.ToArray();var frozenOrbitVertices=sol.OrbitVertices.ToArray();for(var path=0;path<SolarSystemScene.OrbitPathCount;path++)Check(sol.OrbitRootSamples[path*SolarSystemScene.OrbitSampleCount]==bodies[path+1].Position.Value,$"orbit path {path} begins at its evaluated authoritative center");var earthPath=2*SolarSystemScene.OrbitSampleCount;var moonPath=3*SolarSystemScene.OrbitSampleCount;Check(Math.Abs(Math.Sqrt((sol.OrbitRootSamples[earthPath]-sol.OrbitRootSamples[moonPath]).LengthSquared)-Math.Sqrt((bodies[3].Position.Value-bodies[4].Position.Value).LengthSquared))<1e-6,"Moon path preserves Earth-relative hierarchy after root resolution");for(var index=0;index<10;index++){var body=bodies[index];var record=sol.DistantBodies[index];var relative=body.Position.Value-camera.Position.Value;Check((record.Enabled&255u)==(uint)(index+1)&&record.CenterX==(float)relative.X&&record.CenterY==(float)relative.Y&&record.CenterZ==(float)relative.Z,$"label and marker anchor {body.Label} uses evaluated center");}Check((sol.DistantBodies[0].Enabled&0x80000000u)!=0,"focused label/marker metadata");

    var overviewLabels=sol.VisibleLabelIds.ToArray();Check(overviewLabels.Length is >0 and <10&&overviewLabels[0]==SolarSystemBodyIds.Sun.Value&&overviewLabels.Contains(SolarSystemBodyIds.Earth.Value)&&!overviewLabels.Contains(SolarSystemBodyIds.Moon.Value),"overview collision rejection preserves Sun/Earth priority and suppresses overlapping Moon");Check(SolarSystemScene.BodyOrder.All(id=>sol.TryGetLabelBounds(id,out var bounds)&&bounds.IsFinite),"all overview label bounds finite");Check(NonOverlapping(overviewLabels),"accepted overview labels satisfy deterministic clearance margin");for(var index=0;index<10;index++)Check(((sol.DistantBodies[index].Enabled&SolarSystemScene.LabelVisibleBit)!=0)==overviewLabels.Contains(SolarSystemScene.BodyOrder[index]),"native label visibility metadata matches managed selection");sol.Update(camera);Check(overviewLabels.SequenceEqual(sol.VisibleLabelIds.ToArray()),"identical overview camera produces identical label set");Check(frozenOrbitVertices.SequenceEqual(sol.OrbitVertices),"camera-relative solar orbit conversion deterministic");
    ...
    Check(!sol.Focus(camera,NativePresentationFocus.None),"none focus does not select a body");for(var index=0;index<10;index++){var focus=(NativePresentationFocus)(index+1);Check(sol.Focus(camera,focus),$"focus target {focus}");var firstPosition=camera.Position.Value;Check(sol.Focus(camera,focus)&&camera.Position.Value==firstPosition,"deterministic radius-based focus distance");Check(sol.FocusIndex==index&&sol.FocusedBody.BodyId==SolarSystemScene.BodyOrder[index]&&sol.FocusedBlend.Regime==PlanetaryRenderRegime.DetailedOnly&&sol.DetailedComputeRequested&&sol.DistantBodyCount==10,$"focus mapping and promotion {focus}");Check(Math.Sqrt((camera.Position.Value-sol.FocusedBody.Position.Value).LengthSquared)==sol.FocusedBody.RadiusMetres*10d,"positive radius-based focus distance");}
    ...
    var firstCell=SolarOverlayLayout.LabelCellOffset(0,0,0);var nextColumn=SolarOverlayLayout.LabelCellOffset(0,1,0);var nextRow=SolarOverlayLayout.LabelCellOffset(0,0,1);var nextCharacter=SolarOverlayLayout.LabelCellOffset(1,0,0);Check(firstCell==(0d,0d)&&nextColumn.X>firstCell.X&&nextRow.Y>firstCell.Y&&nextCharacter.X>nextColumn.X,"generic Vulkan label progression is left-to-right and top-to-bottom");Check(SolarOverlayLayout.GlyphColumnBit(0)==4&&SolarOverlayLayout.GlyphColumnBit(1)==2&&SolarOverlayLayout.GlyphColumnBit(2)==1,"glyph columns preserve left-to-right bitmap semantics");
    ...
    Check(sol.Focus(camera,3),"Earth focus for interaction");var interactionSnapshot=sol.Presentation;var interactionOrbit=sol.OrbitRootSamples.ToArray();var initialOrbitDistance=sol.OrbitDistance;var initialCamera=camera.Position.Value;sol.ApplyPresentationInput(camera,new NativeInputState{LookActive=1,MouseDeltaX=80,MouseDeltaY=-30},out var rateChanged,out var pauseChanged);Check(!rateChanged&&!pauseChanged&&camera.Position.Value!=initialCamera&&Math.Abs(Math.Sqrt((camera.Position.Value-sol.FocusedBody.Position.Value).LengthSquared)-initialOrbitDistance)<1e-6,"Solar mouse drag orbits current focus");var draggedPosition=camera.Position.Value;sol.ApplyPresentationInput(camera,new NativeInputState{MouseWheelDetents=1},out _,out _);Check(sol.OrbitDistance<initialOrbitDistance&&camera.Position.Value!=draggedPosition,"Solar wheel zoom composes with orbit orientation");
    sol.ApplyPresentationInput(camera,new NativeInputState{RateDecrease=1},out rateChanged,out _);Check(!rateChanged&&sol.Rate==SimulationRate.One,"Solar 1x lower rate clamp");sol.ApplyPresentationInput(camera,new NativeInputState{RateIncrease=1},out rateChanged,out _);Check(rateChanged&&sol.Rate==new SimulationRate(10,1),"Solar established rate step increase");sol.ApplyPresentationInput(camera,new NativeInputState{PauseToggle=1},out _,out pauseChanged);var pausedTime=sol.CurrentTime;var pausedAdvance=sol.TryAdvanceByHostDuration(SimulationDuration.FromWholeSeconds(1),camera,out var pausedError);Check(pauseChanged&&sol.IsPaused&&pausedAdvance&&sol.CurrentTime==pausedTime&&ReferenceEquals(interactionSnapshot,sol.Presentation),$"Solar pause freezes authoritative evaluation: {pausedError}");sol.ApplyPresentationInput(camera,new NativeInputState{PauseToggle=1},out _,out _);Check(!sol.IsPaused,"Solar resume");
    var oldFocusedCenter=sol.FocusedBody.Position.Value;var oldCameraOffset=camera.Position.Value-oldFocusedCenter;Check(sol.TryAdvanceByHostDuration(SimulationDuration.FromWholeSeconds(1),camera,out var advanceError),$"Solar dynamic publication: {advanceError}");Check(sol.CurrentTime==SimulationInstant.FromWholeSeconds(10)&&!ReferenceEquals(interactionSnapshot,sol.Presentation)&&sol.FocusedBody.Position.Value!=oldFocusedCenter,"Solar time advancement republishes evaluated immutable bodies");var newCameraOffset=camera.Position.Value-sol.FocusedBody.Position.Value;Check(Math.Abs(Math.Sqrt(newCameraOffset.LengthSquared)-sol.OrbitDistance)<1e-4&&Math.Sqrt((newCameraOffset-oldCameraOffset).LengthSquared)<1e-4,"focused camera follows moving body without changing orbit offset");Check(interactionSnapshot.Bodies[3].Position.Value==oldFocusedCenter,"prior immutable presentation remains unchanged");Check(interactionOrbit.SequenceEqual(sol.OrbitRootSamples.ToArray()),"time controls leave authoritative sampled trajectory geometry fixed");
...
static void EarthPlanetarySceneTest()
{
    ...
    Check(earthScene.Patches.Length==6*(1<<(2*EarthPlanetaryScene.MaximumLod)),"Earth maximum patch capacity");
    ...
    SetDistance(2d);Console.WriteLine($"Earth adaptive LOD: patches={earthScene.ActivePatchCount}; min={earthScene.MinimumActiveLod}; max={earthScene.MaximumActiveLod}; refined={earthScene.RefinementCount}; balanced={earthScene.BalancedRefinementCount}; culled={earthScene.CulledPatchCount}");Check(earthScene.MinimumActiveLod<earthScene.MaximumActiveLod,"near Earth produces mixed LOD");var closeLeaves=earthScene.ActiveLeaves.ToArray();var closePatches=earthScene.Patches.AsSpan(0,earthScene.ActivePatchCount).ToArray();var closeHash=Hash(closePatches);Check(closeLeaves.Distinct().Count()==closeLeaves.Length,"active patch IDs unique");Check(!HasAncestorOverlap(closeLeaves),"no parent child overlap");Check(IsTraversalOrdered(closeLeaves),"deterministic child traversal order");var axialIndices=closeLeaves.Select((patch,index)=>(patch,index)).Where(item=>item.patch.Face==CubeSphereFace.PositiveZ&&Contains(item.patch,.5d,.5d)).ToArray();Check(axialIndices.Length>0&&axialIndices.Any(item=>closePatches[item.index].StitchMask==0),"near selection retains unstitched camera-axis surface coverage");
    var closest=closeLeaves.MinBy(patch=>PatchDistance(patch));var farthest=closeLeaves.MaxBy(patch=>PatchDistance(patch));Check(closest.Level>farthest.Level,"nearby patch reaches higher LOD than distant patch");var active=closeLeaves.ToHashSet();var edges=new[]{PlanetaryPatchEdge.NegativeU,PlanetaryPatchEdge.PositiveU,PlanetaryPatchEdge.NegativeV,PlanetaryPatchEdge.PositiveV};Check(closeLeaves.All(patch=>edges.All(edge=>PlanetaryRepresentationSelector.FindCoveringNeighbor(patch,edge,active) is not { } neighbor||patch.Level-neighbor.Level<=1)),"edge neighbor level difference at most one");Check(closePatches.Any(patch=>patch.StitchMask!=0),"mixed LOD emits crack-prevention metadata");for(var index=0;index<closeLeaves.Length;index++){uint expectedMask=0;foreach(var edge in edges)if(PlanetaryRepresentationSelector.FindCoveringNeighbor(closeLeaves[index],edge,active) is { } neighbor&&neighbor.Level+1==closeLeaves[index].Level)expectedMask|=(uint)edge;Check(closePatches[index].StitchMask==expectedMask,"deterministic stitch metadata");}
    ...
    SetDistance(1.05d);Check(earthScene.MaximumActiveLod==EarthPlanetaryScene.MaximumLod,"very close Earth respects maximum LOD");
    ...
    Check(EarthPlanetaryScene.TryCreate(root,NativePlanetaryMode.GpuProduction,128,out var gpuScene,out var gpuError)&&gpuScene is not null,$"GPU Earth scene: {gpuError}");var gpuEarth=gpuScene!;var gpuCamera=new CameraState(new FramePosition(root,Double3.Zero),DoubleQuaternion.Identity,gpuEarth.Projection,CameraMode.Free);Check(gpuEarth.TryFocus(gpuCamera),"GPU Earth focus");gpuEarth.UpdatePatches(gpuCamera);var gpuConstants=gpuEarth.GpuConstants(gpuCamera);Check(gpuEarth.Mode==NativePlanetaryMode.GpuProduction&&gpuEarth.ActivePatchCount==0&&!gpuEarth.DetailedComputeRequested&&gpuEarth.DistantDrawCount==1,"distant GPU production suppresses compute and emits one whole-body draw");Check(gpuConstants.CameraBodyX==0&&gpuConstants.CameraBodyY==0&&gpuConstants.CameraBodyZ==(float)(earth.RadiusMetres*EarthPlanetaryScene.InitialOrbitDistanceRadii)&&gpuConstants.Radius==(float)earth.RadiusMetres&&gpuConstants.RefinementThreshold==(float)EarthPlanetaryScene.LodConfiguration.MaximumProjectedPatchSpan&&gpuConstants.MaximumLevel==EarthPlanetaryScene.MaximumLod&&gpuConstants.OutputCapacity==128,"GPU production compact frame constants");SetGpuDistance(15d);Check(gpuEarth.RepresentationBlend.Regime==PlanetaryRenderRegime.Transition&&gpuEarth.DetailedComputeRequested&&gpuEarth.DistantDrawCount==1,"GPU transition enables compute and both renderers");var gpuPresentation=gpuEarth.NativePresentation(gpuCamera);gpuConstants=gpuEarth.GpuConstants(gpuCamera);Check(gpuPresentation.CenterX==-gpuConstants.CameraBodyX&&gpuPresentation.CenterY==-gpuConstants.CameraBodyY&&gpuPresentation.CenterZ==-gpuConstants.CameraBodyZ&&gpuPresentation.Radius==gpuConstants.Radius&&gpuPresentation.DetailedAlpha==gpuConstants.DetailedAlpha,"native distant/detail inputs share center radius and blend");SetGpuDistance(10d);Check(gpuEarth.RepresentationBlend.Regime==PlanetaryRenderRegime.DetailedOnly&&gpuEarth.DetailedComputeRequested&&gpuEarth.DistantDrawCount==0,"GPU detailed-only suppresses distant draw");Check(EarthPlanetaryScene.TryCreate(root,NativePlanetaryMode.CpuGpuValidation,EarthPlanetaryScene.MaximumPatchCapacity,out var validationScene,out var validationError)&&validationScene is not null&&validationScene.ActivePatchCount==0&&validationScene.Mode==NativePlanetaryMode.CpuGpuValidation&&!validationScene.DetailedComputeRequested,$"CPU/GPU oracle distant scene: {validationError}");

    void SetDistance(double radii){camera.Position=camera.Position with{Value=earth.Position.Value+new Double3(0,0,earth.RadiusMetres*radii)};camera.Orientation=DoubleQuaternion.Identity;earthScene.UpdatePatches(camera);}
    void SetGpuDistance(double radii){gpuCamera.Position=gpuCamera.Position with{Value=earth.Position.Value+new Double3(0,0,earth.RadiusMetres*radii)};gpuCamera.Orientation=DoubleQuaternion.Identity;gpuEarth.UpdatePatches(gpuCamera);}
    ...
    static bool IsTraversalOrdered(PlanetaryPatch[] leaves){for(var index=1;index<leaves.Length;index++){var previous=TraversalKey(leaves[index-1]);var current=TraversalKey(leaves[index]);if(previous.CompareTo(current)>0)return false;}return true;}
    static (int Face,int Morton) TraversalKey(PlanetaryPatch patch){var morton=0;for(var bit=patch.Level-1;bit>=0;bit--)morton=(morton<<2)|(((patch.Y>>bit)&1)<<1)|((patch.X>>bit)&1);return((int)patch.Face,morton<<(2*(EarthPlanetaryScene.MaximumLod-patch.Level)));}
    double PatchDistance(PlanetaryPatch patch){var bounds=patch.Bounds;var center=earth.Position.Value+CubeSphereProjection.Project(patch.Face,(bounds.MinX+bounds.MaxX)*.5,(bounds.MinY+bounds.MaxY)*.5,earth.RadiusMetres);return Math.Sqrt((camera.Position.Value-center).LengthSquared);}
...
{
    Check(Marshal.SizeOf<NativeEncodedPosition>() == 32, "encoded size"); Check(Marshal.SizeOf<NativeCameraData>() == 96, "native camera size"); Check(Marshal.OffsetOf<NativeCameraData>(nameof(NativeCameraData.Position)).ToInt32() == 0, "native camera position offset"); Check(Marshal.OffsetOf<NativeCameraData>(nameof(NativeCameraData.ViewProjection)).ToInt32() == 32, "native camera matrix offset"); Check(Marshal.SizeOf<GpuCameraData>() == 96, "GPU camera size"); Check(Marshal.OffsetOf<GpuCameraData>(nameof(GpuCameraData.Position)).ToInt32() == 0, "GPU camera position offset"); Check(Marshal.OffsetOf<GpuCameraData>(nameof(GpuCameraData.ViewProjection)).ToInt32() == 32, "GPU camera matrix offset"); Check(Marshal.SizeOf<NativeRenderTransform>() == 32, "transform size"); Check(Marshal.SizeOf<NativeRenderObject>() == 80, "object stride"); Check(Marshal.OffsetOf<NativeRenderObject>(nameof(NativeRenderObject.Position)).ToInt32() == 0, "position offset"); Check(Marshal.OffsetOf<NativeRenderObject>(nameof(NativeRenderObject.Transform)).ToInt32() == 32, "transform offset"); Check(Marshal.OffsetOf<NativeRenderObject>(nameof(NativeRenderObject.Mesh)).ToInt32() == 64, "mesh offset"); Check(Marshal.SizeOf<NativeDrawBatch>() == 16, "batch stride"); Check(Marshal.SizeOf<NativePlanetaryGpuConstants>() == 48 && Marshal.SizeOf<NativePlanetaryPresentation>() == 48 && Marshal.SizeOf<NativeFrameSubmission>() == 336, "planetary handoff frame ABI sizes"); Check(Marshal.OffsetOf<NativeFrameSubmission>(nameof(NativeFrameSubmission.PlanetaryGpu)).ToInt32() == 208 && Marshal.OffsetOf<NativeFrameSubmission>(nameof(NativeFrameSubmission.PlanetaryMode)).ToInt32() == 256 && Marshal.OffsetOf<NativeFrameSubmission>(nameof(NativeFrameSubmission.PlanetaryPresentation)).ToInt32() == 272 && Marshal.OffsetOf<NativeFrameSubmission>(nameof(NativeFrameSubmission.DistantBodies)).ToInt32() == 320 && Marshal.OffsetOf<NativeFrameSubmission>(nameof(NativeFrameSubmission.DistantBodyCount)).ToInt32() == 328 && Marshal.OffsetOf<NativeFrameSubmission>(nameof(NativeFrameSubmission.DistantBodyPadding)).ToInt32() == 332, "planetary handoff frame ABI offsets"); Check(Marshal.SizeOf<NativeInputState>() == 68 && Marshal.OffsetOf<NativeInputState>(nameof(NativeInputState.SasModeKey)).ToInt32() == 60 && Marshal.OffsetOf<NativeInputState>(nameof(NativeInputState.PresentationFocus)).ToInt32() == 64 && Enum.GetUnderlyingType(typeof(NativePresentationFocus))==typeof(uint), "focus input layout"); Check(NativeRuntime.GetAbiLayout(out var abi) == NativeResult.Success && abi.InputStateSize == 68 && abi.InputSasModeKeyOffset == 60 && abi.InputPresentationFocusOffset == 64 && abi.FrameSubmissionSize == 336 && abi.FramePlanetaryGpuOffset == 208 && abi.FramePlanetaryModeOffset == 256 && abi.FramePlanetaryPresentationOffset == 272, "native frame ABI layout");
}
