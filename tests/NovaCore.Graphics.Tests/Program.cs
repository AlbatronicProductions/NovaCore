...
static void CelestialAnalyticalFixturePublicationTest()
{
    ...
    Check(scene!.CurrentTime == SimulationInstant.Zero && scene.CurrentSnapshot.Count == 2 && scene.CurrentSnapshot.OrbitCurve?.Count == 257 && scene.OrbitCurveBuildCount == 1, "celestial initial snapshot and curve");
    ...
    var cameraRoot = new UniversePosition(new Double3(0, 0, 24), root); var camera = Camera(cameraRoot); var submission = new RenderFrameSubmission(2, 257); Check(ResolvedRenderSubmissionBuilder.TryBuild(scene.CurrentSnapshot, camera, cameraRoot, submission) == ResolvedRenderSubmissionBuildStatus.Success, "celestial submission");
