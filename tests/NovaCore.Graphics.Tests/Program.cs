...
static void EarthPlanetarySceneTest()
{
    ...
    Check(earthScene.ActivePatchCount==6&&earthScene.MinimumActiveLod==0&&earthScene.MaximumActiveLod==0&&earthScene.Representation==PlanetaryRepresentation.NearFieldSurface,"far Earth produces six root patches");
    var farLeaves=earthScene.ActiveLeaves.ToArray();var farHash=Hash(earthScene.Patches.AsSpan(0,earthScene.ActivePatchCount));Check(farLeaves.Select((patch,index)=>patch.Face==(CubeSphereFace)index&&patch.Level==0&&patch.X==0&&patch.Y==0).All(value=>value),"far root patch order");
    SetDistance(10d);Check(earthScene.ActivePatchCount>6&&earthScene.MaximumActiveLod>0,"closer Earth increases active LOD");
    ...
    SetDistance(20d);Check(earthScene.ActivePatchCount==6&&earthScene.ActiveLeaves.SequenceEqual(farLeaves)&&Hash(earthScene.Patches.AsSpan(0,earthScene.ActivePatchCount))==farHash,"receding restores deterministic coarse roots");
    ...
    Check(before.SequenceEqual(earthScene.Presentation.Bodies.ToArray()),"Earth focus and orbit do not mutate presentation snapshot");var relative=CubeSphereProjection.CameraRelativeCenter(earth,new UniversePosition(camera.Position.Value,root));Check(earthScene.Patches.AsSpan(0,earthScene.ActivePatchCount).ToArray().All(patch=>patch.CenterX==(float)relative.X&&patch.CenterY==(float)relative.Y&&patch.CenterZ==(float)relative.Z&&patch.Radius==(float)earth.RadiusMetres),"Earth camera-relative patch batch");
    ...
    Check(EarthPlanetaryScene.TryCreate(root,NativePlanetaryMode.GpuProduction,128,out var gpuScene,out var gpuError)&&gpuScene is not null,$"GPU Earth scene: {gpuError}");var gpuEarth=gpuScene!;var gpuCamera=new CameraState(new FramePosition(root,Double3.Zero),DoubleQuaternion.Identity,gpuEarth.Projection,CameraMode.Free);Check(gpuEarth.TryFocus(gpuCamera),"GPU Earth focus");gpuEarth.UpdatePatches(gpuCamera);var gpuConstants=gpuEarth.GpuConstants(gpuCamera);Check(gpuEarth.Mode==NativePlanetaryMode.GpuProduction&&gpuEarth.ActivePatchCount==0,"GPU production emits no CPU adaptive leaves");Check(gpuConstants.CameraBodyX==0&&gpuConstants.CameraBodyY==0&&gpuConstants.CameraBodyZ==(float)(earth.RadiusMetres*EarthPlanetaryScene.InitialOrbitDistanceRadii)&&gpuConstants.Radius==(float)earth.RadiusMetres&&gpuConstants.RefinementThreshold==(float)EarthPlanetaryScene.LodConfiguration.MaximumProjectedPatchSpan&&gpuConstants.MaximumLevel==EarthPlanetaryScene.MaximumLod&&gpuConstants.OutputCapacity==128,"GPU production compact frame constants");Check(EarthPlanetaryScene.TryCreate(root,NativePlanetaryMode.CpuGpuValidation,EarthPlanetaryScene.MaximumPatchCapacity,out var validationScene,out var validationError)&&validationScene is not null&&validationScene.ActivePatchCount==6&&validationScene.Mode==NativePlanetaryMode.CpuGpuValidation,$"CPU/GPU oracle scene: {validationError}");
