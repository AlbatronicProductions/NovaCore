...
static void EarthPlanetarySceneTest()
{
    ...
    SetDistance(10d);Check(earthScene.ActivePatchCount==24&&earthScene.MinimumActiveLod==1&&earthScene.MaximumActiveLod==1,"closer Earth increases active LOD");var expectedChildren=Enumerable.Range(0,4).Select(index=>new PlanetaryPatch(CubeSphereFace.PositiveX,0,0,0).Child(index)).ToArray();Check(earthScene.ActiveLeaves[..4].SequenceEqual(expectedChildren),"deterministic child order");
    SetDistance(1.05d);Check(earthScene.ActivePatchCount==6*(1<<(2*EarthPlanetaryScene.MaximumLod))&&earthScene.MaximumActiveLod==EarthPlanetaryScene.MaximumLod,"very close Earth respects maximum LOD");var closeLeaves=earthScene.ActiveLeaves.ToArray();var closeHash=Hash(earthScene.Patches.AsSpan(0,earthScene.ActivePatchCount));Check(closeLeaves.Distinct().Count()==closeLeaves.Length,"active patch IDs unique");Check(!HasAncestorOverlap(closeLeaves),"no parent child overlap");
    earthScene.UpdatePatches(camera);Check(closeLeaves.SequenceEqual(earthScene.ActiveLeaves.ToArray())&&closeHash==Hash(earthScene.Patches.AsSpan(0,earthScene.ActivePatchCount)),"repeated camera state has deterministic leaf and batch sequence");
    ...
    static ulong Hash(ReadOnlySpan<NativePlanetaryPatch> patches){ulong hash=14695981039346656037UL;foreach(ref readonly var patch in patches){hash=(hash^patch.Face)*1099511628211UL;hash=(hash^patch.Level)*1099511628211UL;hash=(hash^patch.X)*1099511628211UL;hash=(hash^patch.Y)*1099511628211UL;hash=(hash^(uint)BitConverter.SingleToInt32Bits(patch.CenterX))*1099511628211UL;hash=(hash^(uint)BitConverter.SingleToInt32Bits(patch.CenterY))*1099511628211UL;hash=(hash^(uint)BitConverter.SingleToInt32Bits(patch.CenterZ))*1099511628211UL;hash=(hash^(uint)BitConverter.SingleToInt32Bits(patch.Radius))*1099511628211UL;}Console.WriteLine($"Earth patch batch hash: 0x{hash:X16}");return hash;}
}
