...
public readonly record struct SimulationEventHeader
{
    ...
    public SimulationEventHeader(SimulationEventId id, SimulationInstant time, int priority, SimulationEventSequence sequence, SimulationEventKind kind)
    {
        ...
        if (kind is not (SimulationEventKind.Marker or SimulationEventKind.ReplaceTrajectory or SimulationEventKind.NoOpMarker or SimulationEventKind.CelestialImpulse or SimulationEventKind.RigidBodyTorque or SimulationEventKind.SpacecraftForce or SimulationEventKind.SpacecraftContactImpulse)) throw new ArgumentOutOfRangeException(nameof(kind));
