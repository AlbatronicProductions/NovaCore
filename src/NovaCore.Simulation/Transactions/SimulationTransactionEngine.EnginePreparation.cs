...
internal sealed partial class SimulationTransactionEngine
{
    ...
    private sealed class EnginePreparationStorage(EnginePreparationAuthority authority)
    {
        ...
        internal SpacecraftPhysicalSource Physical;
        ...
        SimulationInstant target, bool refusePreparedForTest, out EngineActuationProposal proposal)
    {
        proposal = default;
        var entered = EnterEnginePreparationPhase(); if (entered != EnginePreparationStatus.Ready) return entered;
        try { return PrepareSingleEngineActuationInOwnedPhase(authority, target, refusePreparedForTest, out proposal); }
        finally { _clock.PublicationPhase.Exit(); }
    }

    private EnginePreparationStatus PrepareSingleEngineActuationInOwnedPhase(EnginePreparationAuthority authority, SimulationInstant target, bool refusePreparedForTest, out EngineActuationProposal proposal)
    {
        proposal = default;
        if (!_clock.PublicationPhase.IsOwnedBy(this)) { proposal = default; return EnginePreparationStatus.ReentrantOperation; }
        if (!OwnsEnginePreparation(authority)) return EnginePreparationStatus.InvalidAuthority;
        ...
        if (!SpacecraftPhysicalSource.TryCapture(view.Spacecraft, commands.Spacecraft, out var physical) || !physical.Properties.IsValid ||
            !view.Spacecraft.TryGetDefinition(commands.Spacecraft, out var craft) || craft.CarrierFrame != commands.RootFrame ||
            (physical.IsEndpoint ? physical.Endpoint.Epoch != start : physical.Linear.Epoch > start || physical.Angular.Epoch > start))
        ...
        p.Preview = preview; p.Clock = clock; p.Physical = physical; p.Craft = craft;
        ...
        EngineActuationProposal proposal, out EngineActuationPreview preview)
    {
        ...
        if (ValidateCommandAuthority(authority.Commands) != SpacecraftCommandStatus.Accepted ||
            ...
            !physical.Same(p.Physical) || craft != p.Craft)
            return EnginePreparationStatus.StaleSource;
