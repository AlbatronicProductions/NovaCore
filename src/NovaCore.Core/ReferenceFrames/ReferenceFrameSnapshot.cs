...
public sealed class ReferenceFrameSnapshot
{
    ...
    private Entry ResolveEntry(ReferenceFrameId id,Dictionary<ReferenceFrameId,(ReferenceFrameDefinition Definition,EvaluatedReferenceFrame Evaluated)> input,HashSet<ReferenceFrameId> path)
    {
        ...
        else {var parent=ResolveEntry(current.Definition.ParentId.Value,input,path);var transform=FrameTransform.Compose(parent.LocalToRoot,current.Evaluated.LocalToParent);var offset=parent.LocalToRoot.Rotation.Rotate(current.Evaluated.LocalToParent.Translation);var originVelocity=parent.OriginVelocityInRoot+parent.LocalToRoot.Rotation.Rotate(current.Evaluated.OriginVelocityInParent)+Double3.Cross(parent.AngularVelocityInRoot,offset);var omega=parent.AngularVelocityInRoot+parent.LocalToRoot.Rotation.Rotate(current.Evaluated.AngularVelocityInParent);result=new(current.Definition,current.Evaluated,transform,originVelocity,omega,parent.IsInertial&&current.Evaluated.IsInertial);}
