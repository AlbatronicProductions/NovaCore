...
bool Digest(const SectorId&id,const Payload&p,std::array<uint8_t,32>&result){
  ...
  auto add=[&](const uint8_t*data,size_t bytes){return BCryptHashData(hash,const_cast<PUCHAR>(data),static_cast<ULONG>(bytes),0)>=0;};
  bool valid=add(identity.data(),24)&&add(p.albedoBc7.data(),p.albedoBc7.size())&&add(p.elevationBc4.data(),p.elevationBc4.size())&&add(p.normalBc5.data(),p.normalBc5.size());
  if(id.payloadVersion>=3u)valid=valid&&add(reinterpret_cast<const uint8_t*>(&p.residualMinimum),sizeof(float))&&add(reinterpret_cast<const uint8_t*>(&p.residualMaximum),sizeof(float))&&add(p.controlR8.data(),p.controlR8.size());
  valid=valid&&BCryptFinishHash(hash,result.data(),static_cast<ULONG>(result.size()),0)>=0;close();return valid;
}
}
...
bool Pack::Open(const std::string&path,std::string&error){
  ...
  version_=Read32(header.data()+8);if(std::memcmp(header.data(),"NCCUBE2\0",8)!=0||(version_!=2u&&version_!=3u)||Read32(header.data()+12)!=HeaderBytes||Read32(header.data()+16)!=RecordHeaderBytes){error="local NCCUBE2 header contract mismatch";return false;}
  ...
  for(uint32_t index=0;index<count;index++){std::array<uint8_t,RecordHeaderBytes>bytes{};if(!input.read(reinterpret_cast<char*>(bytes.data()),bytes.size())){error="local NCCUBE2 record header truncated";return false;}Record record{};record.id={Read64(bytes.data()),Read32(bytes.data()+8),bytes[12],bytes[13],Read32(bytes.data()+16),Read32(bytes.data()+20),bytes[14],bytes[15]};record.payloadOffset=Read64(bytes.data()+24);for(uint32_t channel=0;channel<3;channel++){record.storedBytes[channel]=Read32(bytes.data()+32+channel*4);record.gpuBytes[channel]=Read32(bytes.data()+44+channel*4);record.codecs[channel]=Codec(bytes[56+channel]);}std::memcpy(record.digest.data(),bytes.data()+64,32);record.residualMinimum=residualMinimum_;record.residualMaximum=residualMaximum_;
    if(version_>=3u){record.storedBytes[3]=Read32(bytes.data()+96);record.gpuBytes[3]=Read32(bytes.data()+100);record.codecs[3]=Codec(bytes[60]);record.residualMinimum=ReadFloat(bytes.data()+104);record.residualMaximum=ReadFloat(bytes.data()+108);}
    const uint32_t size=1u<<record.id.level;const bool inconsistentPayload=!records_.empty()&&(record.id.detailFrequency!=records_.front().id.detailFrequency||record.id.payloadVersion!=records_.front().id.payloadVersion);const std::array<uint32_t,4> expectedGpu=version_>=3u?std::array<uint32_t,4>{Bc7Bytes,R16Bytes,Bc5Bytes,R8Bytes}:std::array<uint32_t,4>{Bc7Bytes,Bc4Bytes,Bc5Bytes,0u};const uint32_t channels=version_>=3u?4u:3u;bool channelsValid=record.gpuBytes==expectedGpu&&std::isfinite(record.residualMinimum)&&std::isfinite(record.residualMaximum)&&record.residualMaximum>record.residualMinimum;for(uint32_t channel=0;channel<channels;channel++)channelsValid=channelsValid&&record.storedBytes[channel]!=0u&&(record.codecs[channel]==Codec::Raw||record.codecs[channel]==Codec::PackBits);
    if(record.id.bodyId!=bodyId_||record.id.terrainVersion!=terrainVersion_||record.id.face>=6||record.id.level<minimumLevel_||record.id.level>maximumLevel_||record.id.x>=size||record.id.y>=size||!record.id.detailFrequency||(record.id.payloadVersion!=1u&&record.id.payloadVersion!=2u&&record.id.payloadVersion!=PayloadVersion)||inconsistentPayload||record.payloadOffset!=uint64_t(input.tellg())||!channelsValid){error="local NCCUBE2 record identity/layout invalid";return false;}
    if(std::find_if(records_.begin(),records_.end(),[&](const Record&other){return other.id==record.id;})!=records_.end()){error="local NCCUBE2 sector identity duplicated";return false;}records_.push_back(record);input.seekg(static_cast<std::streamoff>(record.storedBytes[0]+record.storedBytes[1]+record.storedBytes[2]+record.storedBytes[3]),std::ios::cur);if(!input){error="local NCCUBE2 record payload truncated";return false;}}
...
bool Pack::Read(const SectorId&id,Payload&payload,std::string&error)const{
  const auto*record=Find(id);if(!record){error="local terrain sector is outside sparse package";return false;}const auto start=std::chrono::steady_clock::now();std::ifstream input(path_,std::ios::binary);input.seekg(record->payloadOffset);std::array<std::vector<uint8_t>,4>stored;const uint32_t channels=version_>=3u?4u:3u;for(uint32_t channel=0;channel<channels;channel++){stored[channel].resize(record->storedBytes[channel]);if(!input.read(reinterpret_cast<char*>(stored[channel].data()),stored[channel].size())){error="local terrain payload read failed";return false;}}
  payload={};payload.id=id;payload.residualMinimum=record->residualMinimum;payload.residualMaximum=record->residualMaximum;for(uint32_t channel=0;channel<channels;channel++){payload.storedBytes+=record->storedBytes[channel];payload.transcodedBytes+=record->gpuBytes[channel];}if(!Decode(record->codecs[0],stored[0],payload.albedoBc7,record->gpuBytes[0])||!Decode(record->codecs[1],stored[1],payload.elevationBc4,record->gpuBytes[1])||!Decode(record->codecs[2],stored[2],payload.normalBc5,record->gpuBytes[2])||(version_>=3u&&!Decode(record->codecs[3],stored[3],payload.controlR8,record->gpuBytes[3]))){error="local terrain supercompression decode failed";return false;}std::array<uint8_t,32>digest{};payload.digestValid=Digest(id,payload,digest)&&digest==record->digest;payload.transcodeMilliseconds=std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-start).count();if(!payload.digestValid){error="local terrain payload digest mismatch";return false;}return true;
}
