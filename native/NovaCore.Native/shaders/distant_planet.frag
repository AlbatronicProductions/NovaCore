...
void main(){if(stellar!=0u)discard;vec3 albedo=PlanetAlbedo(material.x,normal,color.rgb,response.w);outColor=vec4(PlanetLighting(albedo,normal,lightDirection,viewDirection,response.x,response.y,response.z,lighting.sourceColorAmbient.w),color.a);}
