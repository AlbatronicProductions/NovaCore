...
layout(location=8) flat in uint requestedEarthLevel;
...
void main(){if(stellar!=0u)discard;vec3 surfaceNormal=normalize(normal);vec3 albedo=PlanetAlbedo(material.x,surfaceNormal,color.rgb,response.w);float roughness=response.x,specular=response.y;bool earthData=material.x==10u;vec4 earthAlbedoLand=vec4(albedo,1);float earthElevation=0,earthCloud=0,earthBlend=1;uint earthLevel=0u;uint requestedLevel=requestedEarthLevel;if(earthData){EarthSurfaceSample(surfaceNormal,requestedLevel,0u,earthAlbedoLand,earthElevation,earthCloud,earthBlend,earthLevel);albedo=earthAlbedoLand.rgb;}bool ocean=(environmentData.value.identity.z&4u)!=0u&&(earthData?earthAlbedoLand.a<.5:material.x==3u&&float(PlanetTerrainHeightD(dvec3(surfaceNormal),24u,double(environmentData.value.cloudShape.w)))<environmentData.value.ocean.x);if(ocean){EarthOceanMaterial oceanMaterial=EarthOceanBaseMaterial(albedo,environmentData.value.oceanColorExposure.rgb,environmentData.value.ocean.y,specular);albedo=oceanMaterial.albedo;roughness=oceanMaterial.roughness;specular=oceanMaterial.specular;}if((environmentData.value.identity.z&2u)!=0u&&(material.x==3u||earthData)){float mask,shadowMask;if(earthData){mask=smoothstep(.16,.74,earthCloud);vec4 shadowAlbedo;float shadowElevation,shadowCloud,shadowBlend;uint shadowLevel;EarthSurfaceSample(normalize(surfaceNormal+normalize(lightDirection)*.012),earthLevel,0u,shadowAlbedo,shadowElevation,shadowCloud,shadowBlend,shadowLevel);shadowMask=smoothstep(.16,.74,shadowCloud);}else{float clouds=PlanetFbm(surfaceNormal*environmentData.value.cloudShape.x)+.35*PlanetFbm(surfaceNormal*environmentData.value.cloudShape.y);mask=smoothstep(environmentData.value.clouds.z,1.05,clouds);vec3 shadowDirection=normalize(surfaceNormal+normalize(lightDirection)*.012);float shadowClouds=PlanetFbm(shadowDirection*environmentData.value.cloudShape.x)+.35*PlanetFbm(shadowDirection*environmentData.value.cloudShape.y);shadowMask=smoothstep(environmentData.value.clouds.z,1.05,shadowClouds);}albedo*=1.0-shadowMask*environmentData.value.cloudShape.z;albedo=mix(albedo,vec3(.70,.76,.84),mask*.30);}outColor=vec4(PlanetLighting(albedo,surfaceNormal,lightDirection,viewDirection,roughness,specular,response.z,lighting.sourceColorAmbient.w),color.a);}
