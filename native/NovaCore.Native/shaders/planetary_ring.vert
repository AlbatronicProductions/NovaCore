...
void main(){
  ...
  vec3 local=RotateQuaternion(vec3(inRing.x*radius,0,inRing.y*radius),p.ringOrientation);
  ...
  ringNormal=normalize(RotateQuaternion(vec3(0,1,0),p.ringOrientation));bandFrequency=p.ringGeometry.w;
}
