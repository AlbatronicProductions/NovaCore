...
uint ResolveLayer(vec3 direction,out vec2 localUv)
{
  ...
  for(int signedLevel=int(maximumLevel);signedLevel>=0;signedLevel--)
  {
    ...
    if(layer!=0u){localUv=clamp(uv*float(cells)-vec2(patchCoordinate),vec2(0),vec2(1));return layer;}
  }
  localUv=uv;return 0u;
}
...
void main()
{
  ...
  uint layer;vec2 localUv;float rawHeight=HeightAt(vec3(direction),layer,localUv);double visibleHeight=max(double(rawHeight),double(eye.surface.z));double radius=RadiusD();
  ...
  productionLayer=layer|0x80000000u;productionUv=localUv;
}
