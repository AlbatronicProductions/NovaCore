...
void main(){
  ...
  for(int i=0;i<10;i++){float t=nearT+(float(i)+.5)*stepLength;vec3 p=ro+rd*t;float altitude=length(p-center)-planet;float rayleigh=exp(-max(altitude,0.0)/e.atmosphere.y);float mie=exp(-max(altitude,0.0)/e.atmosphere.z);vec3 local=e.scattering.rgb*rayleigh*rayleighPhase+vec3(e.scattering.w*mie*miePhase);scatter+=local*stepLength*.00012;optical+=(rayleigh*.00000012+mie*.0000004)*stepLength;
    if((e.identity.z&2u)!=0u&&altitude>=e.clouds.x&&altitude<=e.clouds.y){vec3 d=normalize(p-center);float shape=Fbm(d*e.cloudShape.x)+.35*Fbm(d*e.cloudShape.y);float density=smoothstep(e.clouds.z,1.08,shape)*e.clouds.w*stepLength/max(e.clouds.y-e.clouds.x,1.0);float transmittance=1.0-cloudAlpha;float sunTerm=.28+.72*max(dot(d,sun),0.0);cloudColor+=vec3(.62,.69,.80)*density*transmittance*sunTerm;cloudAlpha=clamp(cloudAlpha+density*transmittance,0.0,.70);}}
  float cameraAltitude=length(center)-planet;float inside=1.0-smoothstep(0.0,e.atmosphere.x,cameraAltitude);float localDay=max(dot(normalize(-center),sun),0.0);float atmosphereAlpha=max(clamp(1.0-exp(-optical),0.0,.88),inside*.96);vec3 atmosphereColor=scatter*e.oceanColorExposure.w+vec3(.035,.12,.38)*inside*(.35+.65*localDay);vec3 color=atmosphereColor+cloudColor;float alpha=clamp(atmosphereAlpha+cloudAlpha*(1.0-atmosphereAlpha),0.0,.97);outColor=vec4(color/max(alpha,.001),alpha);
}
