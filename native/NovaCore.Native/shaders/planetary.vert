...
void main(){
  ...
  dvec3 direction=ProjectD(p.address,dvec2(stitched));dvec3 relativePosition;dvec3 surfaceNormal=direction;bool terrain=inputData.controls.z!=0u&&inputData.thresholds.w>0.0;
  if(terrain){uint x=uint(round(stitched.x*16.0)),y=uint(round(stitched.y*16.0));uint slot=patchTerrain.values[gl_InstanceIndex].x;float morph=1.0;double radius=RadiusD();double height=HeightAt(slot,x,y,morph);dvec3 absolutePosition=direction*(radius+height);relativePosition=absolutePosition-CameraD();uint xl=x==0u?0u:x-1u,xr=x==16u?16u:x+1u,yd=y==0u?0u:y-1u,yu=y==16u?16u:y+1u;dvec3 left=ProjectD(p.address,dvec2(xl,y)/16.0)*(radius+HeightAt(slot,xl,y,morph));dvec3 right=ProjectD(p.address,dvec2(xr,y)/16.0)*(radius+HeightAt(slot,xr,y,morph));dvec3 down=ProjectD(p.address,dvec2(x,yd)/16.0)*(radius+HeightAt(slot,x,yd,morph));dvec3 up=ProjectD(p.address,dvec2(x,yu)/16.0)*(radius+HeightAt(slot,x,yu,morph));surfaceNormal=normalize(cross(right-left,up-down));if(dot(surfaceNormal,direction)<0.0)surfaceNormal=-surfaceNormal;}
