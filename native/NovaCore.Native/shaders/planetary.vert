...
dvec3 ProjectD(uvec4 address,dvec2 local){if(inputData.controls.z==4u)return ProductionProjectD(address,local);double cells=double(1u<<address.y);dvec2 uv=(dvec2(address.zw)+local)/cells;return normalize(CubeD(address.x,2.0*uv.x-1.0,2.0*uv.y-1.0));}
dvec3 ProjectGridD(uvec4 address,uvec2 grid){if(inputData.controls.z==4u)return ProductionProjectGridD(address,grid);uint denominator=16u<<address.y;uvec2 numerator=address.zw*16u+grid;dvec2 uv=dvec2(numerator)/double(denominator);return normalize(CubeD(address.x,2.0*uv.x-1.0,2.0*uv.y-1.0));}
...
void main(){
  ...
  if(terrain){uint x=gridCoordinate.x,y=gridCoordinate.y;uint slot=patchTerrain.values[gl_InstanceIndex].x;float morph=p.transitions.y==0u?1.0:clamp(float(inputData.controls.w-p.transitions.y)/30.0,0.0,1.0);double radius=RadiusD();double rawHeight=HeightAt(slot,x,y,morph);terrainHeight=float(rawHeight);double height=rawHeight;dvec3 absolutePosition=direction*(radius+height);relativePosition=absolutePosition-CameraD();if(inputData.controls.z==4u){surfaceNormal=direction;}else{uint xl=x==0u?0u:x-1u,xr=x==16u?16u:x+1u,yd=y==0u?0u:y-1u,yu=y==16u?16u:y+1u;dvec3 left=ProjectGridD(p.address,uvec2(xl,y))*(radius+HeightAt(slot,xl,y,morph));dvec3 right=ProjectGridD(p.address,uvec2(xr,y))*(radius+HeightAt(slot,xr,y,morph));dvec3 down=ProjectGridD(p.address,uvec2(x,yd))*(radius+HeightAt(slot,x,yd,morph));dvec3 up=ProjectGridD(p.address,uvec2(x,yu))*(radius+HeightAt(slot,x,yu,morph));surfaceNormal=normalize(cross(right-left,up-down));if(dot(surfaceNormal,direction)<0.0)surfaceNormal=-surfaceNormal;}}
