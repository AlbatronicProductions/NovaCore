...
void main(){
  ...
  float cells=exp2(float(p.address.y));vec2 uv=(vec2(p.address.zw)+stitched)/cells;float a=2*uv.x-1;float b=2*uv.y-1;vec3 local=normalize(Cube(p.address.x,a,b))*p.centerRadius.w;gl_Position=frameData.camera.viewProjection*vec4(p.centerRadius.xyz+local,1);color=p.color;
}
