...
layout(std430,set=0,binding=12) readonly buffer EyeballDebugInput { vec4 cameraHighRadiusHigh; vec4 cameraLowRadiusLow; vec4 surface; uvec4 identity; vec4 viewForwardMargin; vec4 mapping; uvec4 reserved; } eyeDebug;
...
void main()
{
  ...
  if(earthData)
  {
    uint desired=EarthRequestedLevelForAltitude(planetaryInput.thresholds.z);
