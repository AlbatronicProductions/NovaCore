...
enum NcPlanetaryProductionTopologyFamily : uint32_t {
  NC_PLANETARY_PRODUCTION_TOPOLOGY_RADIAL_NCTOP2 = 0,
  NC_PLANETARY_PRODUCTION_TOPOLOGY_NESTED_SCALE_MESH_NCSM1 = 1
};
struct NcProductionSphericalBillboardSubmission {
  ...
  uint32_t terrainDataGeneration, pupilGeneration;
  float displacementEnvelopeMetres;
  float planetOcclusionSupportRadiusMetres;
  uint32_t topologyFamily;
  uint32_t physicalPreparationMode;
...
static_assert(sizeof(NcProductionSphericalBillboardSubmission) == 96,
              "production billboard submission must match the managed ABI");
struct alignas(32) NcProductionBillboardPupilFrame {
  double east[4];
  double north[4];
  double up[4];
  double transition[4];
...
static_assert(sizeof(NcProductionBillboardPupilFrame) == 160,
              "pupil frame must match std430 layout");
struct alignas(32) NcProductionBillboardFrame {
  NcProductionBillboardPupilFrame previous;
  NcProductionBillboardPupilFrame current;
  NcProductionBillboardPupilFrame incoming;
};
