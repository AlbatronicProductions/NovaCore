...
void PackR16(const std::vector<uint8_t>& source,std::vector<uint32_t>& target){
  if(source.size()%2)throw std::runtime_error("regional R16 byte count invalid");const size_t start=target.size(),count=source.size()/2;target.resize(start+(count+1)/2);
  for(size_t index=0;index<count;index++){uint16_t value{};std::memcpy(&value,source.data()+index*2,2);target[start+index/2]|=uint32_t(value)<<((index&1)*16);}
}
...
NcResult RunPlanetaryHeightQueries(const NcPlanetaryHeightQuery* queries,uint32_t count,NcPlanetaryHeightResult* results,const NcPlanetaryHeightQueryAssets* assets,NcPlanetaryHeightQueryMetrics* metrics){
  // Production preparation is a publication transaction, so retain one
  // bounded dispatch instead of repeatedly reloading the same physical assets.
  // The NCSM1 offline generator independently rejects more than 1.1M leaves;
  // its current finest artifact has 712,106 immutable canonical directions.
  constexpr uint32_t MaximumProductionBillboardQueries=1'100'000u;
  ...
  const auto started=std::chrono::steady_clock::now();try{
    ...
    nc::localterrain::Pack localPack;if(!localPack.Open(assets->localTerrainPathUtf8,error)||!localPack.IsProductionLayout())throw std::runtime_error("regional query pack invalid: "+error);
    std::vector<LocalMetadata> localMetadata;std::vector<uint32_t> localWords;localMetadata.reserve(localPack.RecordCount());localWords.reserve(size_t(localPack.RecordCount())*StoredExtent*StoredExtent/2);
    for(const auto& record:localPack.Records()){
      nc::localterrain::Payload payload;if(!localPack.Read(record.id,payload,error))throw std::runtime_error("regional query payload invalid: "+error);localMetadata.push_back({record.id.face,record.id.level,record.id.x,record.id.y,static_cast<uint32_t>(localMetadata.size()),record.id.detailFrequency,record.id.payloadVersion,0});PackR16(payload.elevationBc4,localWords);
    }
  ...
  }catch(const std::exception& exception){std::fprintf(stderr,"NovaCore physical height query failed: %s\n",exception.what());return NC_FAILURE;}
}
