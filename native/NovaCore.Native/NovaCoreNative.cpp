...
static_assert(sizeof(NcFrameSubmission) == 688);
...
struct App {
  ...
  bool surfaceContextValid{},productionFallbackOwner{},productionSurfaceLogged{};
...
void CompleteProductionUploads(App &a){if(!a.productionPendingUploads)return;auto *words=static_cast<uint32_t*>(a.terrainKeyMapped);auto *lookup=static_cast<uint32_t*>(a.productionLayerLookupMapped);for(uint32_t index=0;index<a.productionPendingUploads;index++){const uint32_t terrainSlot=a.productionUploadTerrainSlots[index],layer=a.productionUploadLayers[index];const uint32_t owner=a.productionLayerTerrainSlot[layer];uint32_t *key=words+terrainSlot*12u;if(owner==terrainSlot&&a.productionLayerGeneration[layer]==a.productionUploadGenerations[index]&&ProductionKeyMatches(key,a.productionUploadRequests[index].id)){key[10]=layer+1u;const auto &id=a.productionUploadRequests[index].id;const uint64_t ordinal=nc::production::Pack::Ordinal(id.face,id.level,id.x,id.y);if(lookup&&ordinal<ProductionLookupCapacity)lookup[ordinal]=layer+1u;a.productionLayerLastUse[layer]=a.frame;}}a.productionPendingUploads=0;}
void QueueProductionRequests(App &a){
  if(!a.productionIo||!a.productionPack||!a.submission||a.submission->planetarySurfaceMode!=NC_PLANETARY_SURFACE_PRODUCTION_CUBE)return;auto *words=static_cast<uint32_t*>(a.terrainKeyMapped);auto &io=*a.productionIo;std::lock_guard lock(io.mutex);
  ...
  for(uint32_t slot=0;slot<TerrainCacheCapacity;slot++){const uint32_t *key=words+slot*12u;if(key[3]==0u||key[10]!=0u||key[11]!=0u)continue;nc::production::PatchId id{uint64_t(key[0])|uint64_t(key[1])<<32,key[2],key[4],key[5],key[6],key[7]};if(!a.productionPack->Contains(id)){words[slot*12u+11u]=3u;continue;}if(io.requestCount==io.requests.size()){io.queueDrops++;a.productionQueueDrops++;break;}io.requests[io.requestTail]={id,slot};io.requestTail=(io.requestTail+1)%io.requests.size();io.requestCount++;words[slot*12u+11u]=1u;a.productionRequests++;}io.wake.notify_all();
}
void PrepareProductionUploads(App &a){
  CompleteProductionUploads(a);if(!a.submission||a.submission->planetarySurfaceMode!=NC_PLANETARY_SURFACE_PRODUCTION_CUBE)return;QueueProductionRequests(a);if(!a.productionIo||a.productionPendingUploads)return;auto &io=*a.productionIo;auto *words=static_cast<uint32_t*>(a.terrainKeyMapped);auto *terrain=static_cast<float*>(a.terrainSampleMapped);std::lock_guard lock(io.mutex);
  for(auto &ready:io.ready){if(a.productionPendingUploads>=ProductionUploadBudget)break;if(ready.state==3u){uint32_t *key=words+ready.request.terrainSlot*12u;if(ProductionKeyMatches(key,ready.request.id))key[11]=3u;ready={};continue;}if(ready.state!=2u||!ready.payload)continue;const uint32_t terrainSlot=ready.request.terrainSlot;uint32_t *key=words+terrainSlot*12u;if(!ProductionKeyMatches(key,ready.request.id)||key[10]!=0u){ready={};continue;}uint32_t layer=UINT32_MAX;for(uint32_t candidate=0;candidate<ProductionPayloadSlots;candidate++)if(a.productionLayerTerrainSlot[candidate]==UINT32_MAX){layer=candidate;break;}if(layer==UINT32_MAX){uint64_t oldest=UINT64_MAX;for(uint32_t candidate=0;candidate<ProductionPayloadSlots;candidate++){const auto &ownerId=a.productionLayerPatch[candidate];if(ownerId.level==0u||a.productionLayerLastUse[candidate]>=a.frame)continue;bool pending=false;for(uint32_t upload=0;upload<a.productionPendingUploads;upload++)pending|=a.productionUploadLayers[upload]==candidate;if(!pending&&a.productionLayerLastUse[candidate]<oldest){oldest=a.productionLayerLastUse[candidate];layer=candidate;}}}if(layer==UINT32_MAX){ready={};key[11]=3u;continue;}if(a.productionLayerTerrainSlot[layer]!=UINT32_MAX){const uint32_t oldTerrainSlot=a.productionLayerTerrainSlot[layer];uint32_t *oldKey=words+oldTerrainSlot*12u;const uint64_t oldOrdinal=nc::production::Pack::Ordinal(a.productionLayerPatch[layer].face,a.productionLayerPatch[layer].level,a.productionLayerPatch[layer].x,a.productionLayerPatch[layer].y);auto *lookup=static_cast<uint32_t*>(a.productionLayerLookupMapped);if(lookup&&oldOrdinal<ProductionLookupCapacity&&lookup[oldOrdinal]==layer+1u)lookup[oldOrdinal]=0u;if(oldKey[10]==layer+1u){oldKey[10]=0u;oldKey[11]=0u;}a.productionEvictions++;}a.productionLayerGeneration[layer]++;a.productionLayerTerrainSlot[layer]=terrainSlot;a.productionLayerPatch[layer]=ready.request.id;a.productionLayerLastUse[layer]=a.frame;a.productionElevationCpu[layer]=ready.payload->elevation;
...
void RecordProductionUploads(App &a,VkCommandBuffer command){
  if(!a.productionPendingUploads||!a.submission||a.submission->planetarySurfaceMode!=NC_PLANETARY_SURFACE_PRODUCTION_CUBE)return;VkImageMemoryBarrier before[3]{};for(uint32_t channel=0;channel<3;channel++){before[channel].sType=VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;before[channel].srcAccessMask=a.productionImagesInitialized?VK_ACCESS_SHADER_READ_BIT:0;before[channel].dstAccessMask=VK_ACCESS_TRANSFER_WRITE_BIT;before[channel].oldLayout=a.productionImagesInitialized?VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL:VK_IMAGE_LAYOUT_UNDEFINED;before[channel].newLayout=VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;before[channel].image=a.productionImages[channel];before[channel].subresourceRange.aspectMask=VK_IMAGE_ASPECT_COLOR_BIT;before[channel].subresourceRange.levelCount=1;before[channel].subresourceRange.layerCount=ProductionPayloadSlots;}vkCmdPipelineBarrier(command,a.productionImagesInitialized?(VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT|VK_PIPELINE_STAGE_VERTEX_SHADER_BIT):VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,VK_PIPELINE_STAGE_TRANSFER_BIT,0,0,nullptr,0,nullptr,3,before);
...
void DestroySubmission(App &a) {
  a.productionLayerTerrainSlot.fill(UINT32_MAX);for(auto &value:a.productionElevationCpu)value.clear();a.productionPendingUploads=0;
  if (a.mapped)
...
void Upload(App &a) {
  ...
  const auto &contextPresentation=a.submission->planetaryPresentation;const uint64_t contextBody=uint64_t(contextPresentation.bodyIdLow)|(uint64_t(contextPresentation.bodyIdHigh)<<32u);uint32_t radiusHighBits{},radiusLowBits{};std::memcpy(&radiusHighBits,&gpuInput.radiusHigh,4);std::memcpy(&radiusLowBits,&gpuInput.radiusLow,4);const uint32_t contextMode=static_cast<uint32_t>(a.submission->planetarySurfaceMode),contextRegime=static_cast<uint32_t>(contextPresentation.regime);const bool contextChanged=!a.surfaceContextValid||a.surfaceContextBodyId!=contextBody||a.surfaceContextTerrainVersion!=gpuInput.terrainVersion||a.surfaceContextMode!=contextMode||a.surfaceContextRegime!=contextRegime||a.surfaceContextRadiusHighBits!=radiusHighBits||a.surfaceContextRadiusLowBits!=radiusLowBits;if(contextChanged){a.surfaceContextValid=true;a.surfaceContextBodyId=contextBody;a.surfaceContextTerrainVersion=gpuInput.terrainVersion;a.surfaceContextMode=contextMode;a.surfaceContextRegime=contextRegime;a.surfaceContextRadiusHighBits=radiusHighBits;a.surfaceContextRadiusLowBits=radiusLowBits;a.surfaceTransitionEpoch++;a.surfaceContextInvalidations++;a.productionFallbackOwner=productionSurface;std::memset(a.gpuControlMapped,0,sizeof(GpuPlanetaryControl));a.hasGpuTelemetry=false;char transition[320];std::snprintf(transition,sizeof transition,"Planetary context transition: epoch=%llu; body=%llu; surfaceMode=%u; terrainVersion=%u; regime=%u; productionEligible=%s; fallbackOwner=%s",(unsigned long long)a.surfaceTransitionEpoch,(unsigned long long)contextBody,contextMode,gpuInput.terrainVersion,contextRegime,productionSurface?"true":"false",a.productionFallbackOwner?"true":"false");a.Log(NC_LOG_ALWAYS,transition);}else if(productionSurface)a.productionDemandHits++;else a.productionDemandMisses++;
  if(productionSurface&&!a.productionSurfaceLogged){a.Log(NC_LOG_ALWAYS,"Production surface: terrain-v4 body-fixed relaxed cube-sphere; real NCCUBE payloads; legacy equirectangular resources disabled");a.productionSurfaceLogged=true;}std::memcpy(a.gpuInputMapped,&gpuInput,sizeof(gpuInput));
...
void Record(App &a, uint32_t image) {
  ...
  if(regional&&(a.submission->planetaryPatchCount||gpuPlanetary)){VkDeviceSize offset=0;vkCmdBindPipeline(c,VK_PIPELINE_BIND_POINT_GRAPHICS,production?a.productionPlanetaryPipeline:a.planetaryPipeline);vkCmdBindVertexBuffers(c,0,1,&a.planetaryPatch.vb,&offset);vkCmdBindIndexBuffer(c,a.planetaryPatch.ib,0,VK_INDEX_TYPE_UINT32);if(gpuPlanetary)vkCmdDrawIndexedIndirect(c,a.gpuControlBuffer,0,1,sizeof(VkDrawIndexedIndirectCommand));else vkCmdDrawIndexed(c,a.planetaryPatch.indices,a.submission->planetaryPatchCount,0,0,0);}
