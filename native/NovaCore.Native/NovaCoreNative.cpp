...
constexpr uint32_t EarthTileSize=256,EarthTileGutter=2,EarthTileExtent=260,EarthMaximumLevel=4,
                   EarthTileCount=682,EarthPhysicalSlots=128,EarthChannelCount=4,EarthUploadBudget=4,EarthMaximumPendingUploads=8,
...
constexpr std::array<uint32_t,EarthChannelCount> EarthChannelMaximumLevels{4,4,4,2};
constexpr std::array<uint32_t,EarthChannelCount> EarthChannelTileCounts{682,682,682,42};
constexpr uint64_t EarthPackBytes=162781056;
constexpr std::array<uint64_t,EarthChannelCount> EarthChannelOffsets{256,46103456,138309856,161361456};
constexpr std::array<uint8_t,32> EarthPackIdentity{0x66,0x4f,0xf3,0x2c,0x3a,0x57,0x04,0x39,0x60,0xf2,0x46,0xd5,0xd9,0x73,0x97,0x21,0x4c,0xed,0xc4,0xb9,0x76,0xe4,0x8e,0x86,0x7e,0x98,0x03,0xc4,0x14,0xd7,0x96,0xb5};
constexpr std::array<uint8_t,32> EarthPackPayload{0xd0,0x9a,0x9d,0xdf,0x94,0x42,0x42,0xa7,0xd3,0x22,0xae,0x3c,0xe5,0x8c,0x1b,0x0b,0x31,0x01,0x4f,0xeb,0x8d,0x6a,0x33,0x0f,0xb9,0xd5,0x92,0xe4,0x38,0xe9,0xd3,0x06};
...
void CreateEarthVirtualTexture(App &a){
  ...
  if(sourceAvailable){a.earthIo=std::make_unique<EarthIoState>();a.earthIo->path=path;a.earthIo->offsets=channelOffsets;a.earthIo->worker=std::thread(EarthIoWorker,a.earthIo.get());}a.earthAvailable=true;uint64_t poolBytes=0;for(uint32_t value:a.earthRuntimeTileBytes)poolBytes+=uint64_t(value)*EarthPhysicalSlots;char message[320];std::snprintf(message,sizeof message,"Earth VT v3: source=%s; channels=4; LOD=(4,4,4,2); poolSlots=%u/channel; poolBytes=%llu; staging=%u; uploadBudget=%u; identity=664ff32c3a570439",sourceAvailable?"BC7+BC4+R16":"procedural-uncompressed",EarthPhysicalSlots,static_cast<unsigned long long>(poolBytes),EarthStagingBytes,EarthUploadBudget);a.Log(NC_LOG_RENDERER,message);
}
void DestroyEarthVirtualTexture(App &a){if(a.earthIo){{std::lock_guard lock(a.earthIo->mutex);a.earthIo->stop=true;}a.earthIo->wake.notify_all();if(a.earthIo->worker.joinable())a.earthIo->worker.join();a.earthIo.reset();}DestroyHostBuffer(a,a.earthStagingBuffer,a.earthStagingMemory,a.earthStagingMapped);DestroyHostBuffer(a,a.earthPageBuffer,a.earthPageMemory,a.earthPageMapped);if(a.earthSampler)vkDestroySampler(a.device,a.earthSampler,nullptr);for(uint32_t channel=0;channel<EarthChannelCount;channel++){if(a.earthImageViews[channel])vkDestroyImageView(a.device,a.earthImageViews[channel],nullptr);if(a.earthImages[channel])vkDestroyImage(a.device,a.earthImages[channel],nullptr);if(a.earthImageMemory[channel])vkFreeMemory(a.device,a.earthImageMemory[channel],nullptr);}a.earthAvailable=false;}
...
void CreateSubmission(App &a) {
  ...
  VkDescriptorPoolSize ps[3]{{VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,15},{VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,1},{VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,4}};
