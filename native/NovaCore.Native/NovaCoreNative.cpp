...
struct App {
  ...
  bool surfaceContextValid{},productionSurfaceLogged{},productionRootsReadyLogged{};uint32_t earthTransitionTraceRemaining{},earthSubmissionTraceRemaining{};uint64_t recordSerial{},submitSerial{},presentSerial{};
...
void CompleteProductionUploads(App &a){
  ...
  if(!a.productionRootsReadyLogged&&lookup){
    ...
    if(rootMask==0x3fu){a.Log(NC_LOG_ALWAYS,"Earth terrain-v4 material roots ready: mask=0x3F; renderer-lifetime pinned=true");a.productionRootsReadyLogged=true;}
  }
}
...
void BootstrapProductionRoots(App &a){
  if(!a.productionPack)return;
  if(!a.terrainKeyMapped||!a.terrainSampleMapped||!a.productionLayerLookupMapped||!a.productionStagingMapped)throw std::runtime_error("production root bootstrap resources unavailable");
  ...
  for(uint32_t firstFace=0;firstFace<6;firstFace+=ProductionMaximumPendingUploads){
    a.productionPendingUploads=0;
    for(uint32_t face=firstFace;face<std::min<uint32_t>(6,firstFace+ProductionMaximumPendingUploads);face++){
      const nc::production::PatchId id{nc::production::EarthBodyId,a.productionPack->TerrainVersion(),face,0,0,0};nc::production::Payload payload;std::string error;if(!a.productionPack->Read(id,payload,error)||!payload.digestValid)throw std::runtime_error("Production root bootstrap failed for face "+std::to_string(face)+": "+error);
      const uint32_t batch=a.productionPendingUploads,layer=face,terrainSlot=face;uint32_t *key=words+terrainSlot*12u;a.productionLayerGeneration[layer]++;a.productionLayerTerrainSlot[layer]=terrainSlot;a.productionLayerPatch[layer]=id;a.productionLayerLastUse[layer]=a.frame;a.productionElevationCpu[layer]=payload.elevation;
      ...
      for(uint32_t y=0;y<17;y++)for(uint32_t x=0;x<17;x++){const uint32_t sample=(terrainSlot*TerrainGridVertexCount+y*17u+x)*2u;const float elevation=ProductionSampleElevation(payload.elevation,double(x)*16.0,double(y)*16.0);terrain[sample]=elevation;terrain[sample+1]=elevation;}
  ...
  vkFreeCommandBuffers(a.device,a.pool,1,&command);if(!ProductionRootPayloadsReady(a))throw std::runtime_error("production root bootstrap did not publish all six faces");a.Log(NC_LOG_ALWAYS,"Earth terrain-v4 startup roots synchronously resident before first submitted presentation frame");
}
...
void CreateTerrainResidency(App &a) {
  ...
  // The six terrain-v4 roots are renderer-lifetime material dependencies of
  // every Earth representation, including the distant bootstrap sphere.
  // Seed them before focus selection so body switching cannot change which
  // fundamental Earth material is available.
  if(a.productionPack){auto *words=static_cast<uint32_t*>(a.terrainKeyMapped);for(uint32_t face=0;face<6;face++){uint32_t *key=words+face*12u;key[0]=6u;key[1]=0u;key[2]=a.productionPack->TerrainVersion();key[3]=a.productionPack->TerrainVersion();key[4]=face;key[5]=0u;key[6]=0u;key[7]=0u;}}
...
void Upload(App &a) {
  ...
  if(contextChanged){
    if(productionSurface&&!ProductionRootPayloadsReady(a))throw std::runtime_error("production context selected before all six root payloads were resident");
    a.surfaceContextValid=true;a.surfaceContextBodyId=contextBody;a.surfaceContextTerrainVersion=gpuInput.terrainVersion;a.surfaceContextMode=contextMode;a.surfaceContextRegime=contextRegime;a.surfaceContextRadiusHighBits=radiusHighBits;a.surfaceContextRadiusLowBits=radiusLowBits;a.surfaceTransitionEpoch++;a.surfaceContextInvalidations++;a.earthTransitionTraceRemaining=contextBody==nc::production::EarthBodyId?120u:0u;a.earthSubmissionTraceRemaining=a.earthTransitionTraceRemaining;std::memset(a.gpuControlMapped,0,sizeof(GpuPlanetaryControl));a.hasGpuTelemetry=false;
    char transition[320];std::snprintf(transition,sizeof transition,"Planetary context transition: epoch=%llu; body=%llu; surfaceMode=%u; terrainVersion=%u; regime=%u; productionEligible=%s; owner=%s",(unsigned long long)a.surfaceTransitionEpoch,(unsigned long long)contextBody,contextMode,gpuInput.terrainVersion,contextRegime,productionSurface?"true":"false",productionSurface?"terrain-v4":"bounded-sphere");a.Log(NC_LOG_ALWAYS,transition);
    if(contextBody==6u){
...
void Record(App &a, uint32_t image) {
  ...
  const auto &presentation=a.submission->planetaryPresentation;const bool production=a.submission->planetarySurfaceMode==NC_PLANETARY_SURFACE_PRODUCTION_CUBE;const bool handoff=presentation.enabled!=0;const bool detailedPresentation=!handoff||presentation.regime!=NC_PLANETARY_DISTANT_ONLY;const bool distantPresentation=handoff&&!production&&presentation.regime!=NC_PLANETARY_DETAILED_ONLY&&presentation.distantAlpha>0;const bool productionEyeball=production&&a.submission->planetaryEyeball.enabled!=0&&a.productionEyeballPromotion>0;const bool regional=production||detailedPresentation;const bool gpuPlanetary=regional&&a.submission->planetaryMode!=NC_PLANETARY_CPU_REFERENCE;
...
void Draw(App &a) {
  ...
  const auto submitStart=std::chrono::steady_clock::now();a.Check(vkQueueSubmit(a.graphicsQueue, 1, &si, a.fence), "submit failed");a.submitSerial++;const auto submitEnd=std::chrono::steady_clock::now();
