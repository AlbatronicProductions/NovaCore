...
internal sealed class EarthPlanetaryScene
{
    ...
    internal static readonly PlanetaryTerrainDefinition Terrain = PlanetaryTerrainDefinition.EarthProductionCubeV5;
    ...
    internal void SetValidationAltitude(CameraState camera,double altitudeMetres,string surfaceSite="land")
    {
        ...
        var direction=ValidationSurfaceDirection(surfaceSite);_orbitYawRadians=Math.Atan2(direction.X,direction.Z);_orbitPitchRadians=-Math.Asin(direction.Y);_surfaceFocus=null;_localYawRadians=0d;_localPitchRadians=surfaceSite=="local-payload"?-Math.PI*89d/180d:-Math.PI/12d;_cameraPresentationMode=PlanetaryCameraPresentationMode.Orbital;var surfaceRadius=PlanetaryTerrainQuery.VisibleSurfaceRadius(_earth.RadiusMetres,direction,Terrain,EarthSeaLevelMetres);_orbitDistance=surfaceRadius+altitudeMetres;ApplyOrbitPose(camera);
    }

    internal static Double3 ValidationSurfaceDirection(string surfaceSite)
    {
        return surfaceSite switch{"land"=>AtLatitudeLongitude(-45d,-70d),"ocean"=>AtLatitudeLongitude(-30d,-90d),"slope"=>AtLatitudeLongitude(-25d,-70d),"local-payload"=>RelaxedCubeSphereProjection.UnitDirection(CubeSphereFace.PositiveZ,2047.5d/4096d,864.5d/4096d),"arid"=>AtLatitudeLongitude(24.0d,12.0d),"temperate"=>AtLatitudeLongitude(48.0d,12.0d),"rock"=>AtLatitudeLongitude(-25d,-70d),"snow"=>AtLatitudeLongitude(77.0d,-42.0d),"fallback"=>AtLatitudeLongitude(-27.0d,133.0d),"florida"=>AtLatitudeLongitude(FloridaLaunchSite.Latitude,FloridaLaunchSite.Longitude),_=>throw new ArgumentOutOfRangeException(nameof(surfaceSite))};
    ...
    internal void UpdatePatches(CameraState camera)
    {
        ...
        if(_mode==NativePlanetaryMode.GpuProduction){_activeLeaves=[];_activePatchCount=0;_minimumActiveLod=0;_maximumActiveLod=0;_refinementCount=0;_balancedRefinementCount=0;_culledPatchCount=0;_frustumCulledPatchCount=0;_horizonCulledPatchCount=0;_splitPatchCount=0;_mergedPatchCount=0;return;}
    ...
    internal NativePlanetaryGpuConstants GpuConstants(CameraState camera,double viewportHeightPixels=ProofViewportHeightPixels)
    {
    ...
    internal NativePlanetaryPresentation NativePresentation(CameraState camera)
    {
        ...
        var native = new NativePlanetaryPresentation{CenterX=encodedCenter.HighX,CenterY=encodedCenter.HighY,CenterZ=encodedCenter.HighZ,Radius=(float)_earth.RadiusMetres,CenterLowX=encodedCenter.LowX,CenterLowY=encodedCenter.LowY,CenterLowZ=encodedCenter.LowZ,ColorR=_earth.Color.X,ColorG=_earth.Color.Y,ColorB=_earth.Color.Z,DistantAlpha=0f,DetailedAlpha=1f,DistanceRadii=(float)_blend.DistanceRadii,Regime=NativePlanetaryRenderRegime.DetailedOnly,Enabled=1};
