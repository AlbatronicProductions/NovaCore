...
static unsafe int Run(SampleOptions options,LogOptions log,bool productionEarth)
{
    ...
    ResolvedRenderSnapshot? snapshot=null; FixtureSceneDiagnostics fixture=default; DynamicReferenceFrameFixtureScene? dynamic=null; CelestialAnalyticalScene? celestial=null; EarthPlanetaryScene? earth=null; SolarSystemScene? sol=null; var cameraPosition=defaultStart.Value;var cameraOrientation=DoubleQuaternion.Identity; var projection=new CameraProjection(Math.PI/3,16d/9,.01,1000); var movementSpeed=.1d;
    ...
    var objectCapacity=snapshot!.Count+(sol is null?0:2);var state=new HostState(camera,frames,new ReferenceFrameResolver(frames),snapshot!,new RenderFrameSubmission(objectCapacity,Math.Max(snapshot.OrbitCurve?.Count??0,sol?.OrbitVertices.Length??0)),log,movementSpeed,dynamic,celestial,earth,planetaryPatches,sol,options.BenchmarkFrames,options.SolarWarpTraversal);
