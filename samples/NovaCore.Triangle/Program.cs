...
static unsafe int Run(SampleOptions options,LogOptions log,bool productionEarth)
{
    ...
    if(options.Scene=="stock-assembly")
    {
        assembly=new(options.StockDesign);snapshot=assembly.InitialSnapshot;cameraPosition=new(2,3,8);
    ...
    var objectCapacity=assembly?.RenderCapacity??(snapshot!.Count+(sol is null?0:2));var state=new HostState(camera,frames,new ReferenceFrameResolver(frames),snapshot!,new RenderFrameSubmission(objectCapacity,Math.Max(snapshot!.OrbitCurve?.Count??0,sol?.OrbitVertices.Length??0)),log,movementSpeed,dynamic,celestial,earth,planetaryPatches,sol,options.BenchmarkFrames,options.SolarWarpTraversal);state.Contact=contact;state.Powered=powered;state.PoweredContact=poweredContact;state.Assembly=assembly;state.AssemblyAutoStart=options.BenchmarkFrames>0;
...
static unsafe void Callback(NativeHostEvent* e,IntPtr data)
{
    ...
    s.Assembly?.AdvanceLive(e->Input.PauseToggle!=0||s.AssemblyAutoStart);s.AssemblyAutoStart=false;
