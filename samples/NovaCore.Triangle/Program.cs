...
static unsafe int Run(SampleOptions options,LogOptions log)
{
    ...
    if(options.Scene=="earth")
    {
        ...
        dynamicAnchored=new(earth.Earth.BodyId,earth.Earth.RadiusMetres,EarthPlanetaryScene.Terrain,(uint)PlanetaryPhysicalSurface.ModifierGenerationId);
    }
    else if(options.Scene=="sol")
    {
        ...
        dynamicAnchored=new(earthBody.BodyId,earthBody.RadiusMetres,EarthPlanetaryScene.Terrain,(uint)PlanetaryPhysicalSurface.ModifierGenerationId);
    ...
    if(sol is not null&&options.SurfaceSite=="florida-launch"&&!sol.TryStartAtFloridaLaunchSite(camera)){Console.Error.WriteLine("Florida launch-site camera startup failed.");return 1;}
    ...
    var objectCapacity=snapshot!.Count+(sol is null?0:1);var state=new HostState(camera,frames,new ReferenceFrameResolver(frames),snapshot!,new RenderFrameSubmission(objectCapacity,Math.Max(snapshot.OrbitCurve?.Count??0,sol?.OrbitVertices.Length??0)),log,movementSpeed,dynamic,celestial,earth,planetaryPatches,sol,dynamicAnchored,options.BenchmarkFrames,options.DynamicTraversal,options.SolarWarpTraversal);
...
file sealed class HostState(CameraState camera,ReferenceFrameSnapshot frames,ReferenceFrameResolver resolver,ResolvedRenderSnapshot snapshot,RenderFrameSubmission submission,LogOptions log,double movementSpeed,DynamicReferenceFrameFixtureScene? dynamic,CelestialAnalyticalScene? celestial,EarthPlanetaryScene? earth,NativePlanetaryPatch[] planetaryPatches,SolarSystemScene? sol,PlanetaryDynamicAnchoredSurface? dynamicAnchored,int benchmarkFrames,bool dynamicTraversal,bool solarWarpTraversal){public CameraState Camera=camera;public CameraState Default=new(new FramePosition(camera.Position.Frame,camera.Position.Value),camera.Orientation,camera.Projection,camera.Mode);public ReferenceFrameSnapshot Frames=frames;public ReferenceFrameResolver Resolver=resolver;public ResolvedRenderSnapshot Snapshot=snapshot;public RenderFrameSubmission Submission=submission;public LogOptions Log=log;public FreeCameraController Controller=new(movementSpeed,.002,Math.PI*89/180);public DynamicReferenceFrameFixtureScene? Dynamic=dynamic;public CelestialAnalyticalScene? Celestial=celestial;public EarthPlanetaryScene? Earth=earth;public SolarSystemScene? Sol=sol;public PlanetaryDynamicAnchoredSurface? DynamicAnchored=dynamicAnchored;public DynamicAnchoredVulkanTraversal? DynamicTraversal=dynamicTraversal?new():null;public SolarWarpVulkanTraversal? SolarWarpTraversal=solarWarpTraversal?new():null;public NativePlanetaryPatch[] PlanetaryPatches=planetaryPatches;public int BenchmarkFramesRemaining=benchmarkFrames;public bool DynamicFailureLogged;public bool CelestialFailureLogged;public bool SolarFailureLogged;public bool HasSasIndicatorDiagnostic;public bool LastSasTargetValid;public long NextSasIndicatorDiagnosticTimestamp;public long NextSolarDiagnosticTimestamp;public bool HasEarthLodDiagnostic;public int LastEarthPatchCount;public int LastEarthMinimumLod;public int LastEarthMaximumLod;public PlanetaryRepresentation LastEarthRepresentation;public PlanetaryRenderRegime LastEarthRegime;public uint LastDynamicAnchoredGeneration=uint.MaxValue;public int LastDynamicAnchoredDemand=-1;public bool LastDynamicAnchoredVisible;public bool DynamicAnchoredProbeLogged;public uint ViewportWidthPixels=(uint)Math.Round(camera.Projection.AspectRatio*EarthPlanetaryScene.ProofViewportHeightPixels);public uint ViewportHeightPixels=(uint)EarthPlanetaryScene.ProofViewportHeightPixels;}
file readonly record struct SampleOptions(int ObjectCount,string Scene,NativePlanetaryMode PlanetaryMode,uint GpuCapacity,double? AltitudeMetres,int BenchmarkFrames,string SurfaceSite,bool SolarJ2000,PlanetarySurfaceRendererMode PlanetSurfaceMode,bool DynamicTraversal,bool SolarWarpTraversal,string[] LogArguments){public static bool TryParse(string[] a,out SampleOptions v,out string? e){var n=100;var scene="grid";var mode=NativePlanetaryMode.GpuProduction;var surfaceMode=PlanetarySurfaceRendererMode.ProductionCubeSphere;uint capacity=EarthPlanetaryScene.MaximumPatchCapacity;double? altitude=null;var benchmarkFrames=0;var surfaceSite="land";var solarJ2000=false;var dynamicTraversal=false;var solarWarpTraversal=false;var logs=new List<string>();foreach(var x in a){if(x.StartsWith("--objects=")&&(!int.TryParse(x[10..],out n)||n is not(1 or 100 or 1000 or 10000))){v=default;e="Usage: --objects=1|100|1000|10000";return false;}if(x.StartsWith("--scene="))scene=x[8..];else if(x.StartsWith("--planetary-mode=")){mode=x[17..] switch{"cpu"=>NativePlanetaryMode.CpuReference,"gpu"=>NativePlanetaryMode.GpuProduction,"validate"=>NativePlanetaryMode.CpuGpuValidation,_=>(NativePlanetaryMode)uint.MaxValue};}else if(x.StartsWith("--planet-surface=")){surfaceMode=x[17..] switch{"production"=>PlanetarySurfaceRendererMode.ProductionCubeSphere,_=>(PlanetarySurfaceRendererMode)uint.MaxValue};}else if(x.StartsWith("--gpu-capacity=")&&!uint.TryParse(x[15..],out capacity)){v=default;e="GPU capacity must be an integer.";return false;}else if(x.StartsWith("--altitude=")&&(!double.TryParse(x[11..],System.Globalization.NumberStyles.Float,System.Globalization.CultureInfo.InvariantCulture,out var parsedAltitude)||parsedAltitude<EarthPlanetaryScene.MinimumTerrainClearanceMetres)){v=default;e="Altitude must be at least the terrain-safe floor.";return false;}else if(x.StartsWith("--altitude="))altitude=double.Parse(x[11..],System.Globalization.CultureInfo.InvariantCulture);else if(x.StartsWith("--surface-site="))surfaceSite=x[15..];else if(x=="--solar-epoch=j2000")solarJ2000=true;else if(x.StartsWith("--solar-epoch=")){v=default;e="Solar epoch must be current or j2000.";return false;}else if(x=="--dynamic-traversal")dynamicTraversal=true;else if(x=="--solar-warp-traversal")solarWarpTraversal=true;else if(x.StartsWith("--benchmark-frames=")&&(!int.TryParse(x[19..],out benchmarkFrames)||benchmarkFrames<2)){v=default;e="Benchmark frames must be at least 2.";return false;}else logs.Add(x);}var earthSite=surfaceSite is "land"or"ocean"or"slope"or"local-payload"or"arid"or"temperate"or"rock"or"snow"or"fallback"or"florida";var floridaSite=surfaceSite=="florida-launch"&&scene=="sol";if(scene is not("grid"or"frames"or"fixture"or"fixture-dynamic"or"celestial"or"planetary-subdivision-diagnostic"or"earth"or"sol")||mode>NativePlanetaryMode.CpuGpuValidation||surfaceMode!=PlanetarySurfaceRendererMode.ProductionCubeSphere||capacity is 0 or >EarthPlanetaryScene.MaximumPatchCapacity||altitude.HasValue&&scene!="earth"||dynamicTraversal&&scene!="earth"||solarWarpTraversal&&scene!="sol"||!earthSite&&!floridaSite){v=default;e="Usage: --scene=earth [--dynamic-traversal] or --scene=sol [--solar-warp-traversal] [--surface-site=florida-launch] or --scene=earth [--surface-site=land|ocean|slope|local-payload|arid|temperate|rock|snow|fallback] [--altitude=metres] or --scene=planetary-subdivision-diagnostic";return false;}v=new(n,scene,mode,capacity,altitude,benchmarkFrames,surfaceSite,solarJ2000,surfaceMode,dynamicTraversal,solarWarpTraversal,logs.ToArray());e=null;return true;}}
