...
static unsafe int Run(SampleOptions options,LogOptions log,bool productionEarth)
{
    ...
    else if(options.Scene=="sol")
    {
        ...
        if(options.UsesFloridaSlab)assembly=new(options.StockDesign,supportedContact:true,floridaSupport:true,solarWorld:sol);
    ...
    if(sol is not null&&assembly?.FloridaView?.SolarNavigation==true)
    {
    ...
    var objectCapacity=assembly?.RenderCapacity??(snapshot!.Count+(sol is null?0:2));var state=new HostState(camera,frames,new ReferenceFrameResolver(frames),snapshot!,new RenderFrameSubmission(objectCapacity,Math.Max(snapshot!.OrbitCurve?.Count??0,sol?.OrbitVertices.Length??0)),log,movementSpeed,dynamic,celestial,earth,planetaryPatches,sol,options.BenchmarkFrames,options.SolarWarpTraversal);state.Contact=contact;state.Powered=powered;state.PoweredContact=poweredContact;state.Assembly=assembly;state.AssemblyAutoStart=options.BenchmarkFrames>0||assembly?.FloridaView?.SolarNavigation==true;
...
static unsafe void Callback(NativeHostEvent* e,IntPtr data)
{
    ...
    var celestialScene=s.Celestial;var earthScene=s.Earth;var orbitCamera=celestialScene is not null||earthScene is not null||s.Sol is not null&&(s.Assembly?.FloridaView is null||s.Assembly.FloridaView.SolarNavigation);Span<CameraCommand> commands=stackalloc CameraCommand[4];var count=DebugCameraInput.Map(e->Input,commands,!orbitCamera,!orbitCamera);var previousSpeed=s.Controller.MovementSpeed;
    ...
    if(s.Sol is { } vesselSolar&&s.Assembly?.FloridaView?.SolarNavigation==true)
