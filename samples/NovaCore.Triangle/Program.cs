...
static unsafe int Run(SampleOptions options,LogOptions log,bool productionBillboardCandidate)
{
    ...
    var candidateLattice=Array.Empty<NativeProductionBillboardLatticeVertex>();var candidateIndices=Array.Empty<uint>();var candidatePhysical=Array.Empty<NativeSphericalBillboardPhysicalVertex>();
    var candidateLevel=0u;var candidateLatticeScale=0u;var candidatePhysicalGeneration=0u;var candidateTerrainGeneration=0u;var candidatePupilGeneration=0u;var candidateTopologyHash=0ul;
    if(productionBillboardCandidate)
    {
        ...
        var selection=selector.Evaluate(new(candidateAltitude,cameraBody.Normalized(),(int)state.ViewportWidthPixels,(int)state.ViewportHeightPixels,camera.Projection.VerticalFieldOfViewRadians,0),false);
        var topology=levels[selection.Level];var pupil=PlanetaryProductionBillboardPupil.Resolve(default,cameraBody.Normalized(),topology);
        var physical=PlanetarySphericalBillboardNaturalTerrainProof.PrepareProduction(repositoryRoot,topology,pupil);
        candidateLattice=topology.Vertices.Select(vertex=>new NativeProductionBillboardLatticeVertex{CubeX=vertex.CubeX,CubeY=vertex.CubeY,CubeZ=vertex.CubeZ,Metadata=(uint)vertex.DensityRegion|((uint)vertex.RefinementDepth<<8)}).ToArray();
        candidateIndices=topology.Indices.ToArray();candidatePhysical=physical.Vertices;candidateLevel=(uint)topology.Level;candidateLatticeScale=(uint)topology.LatticeScale;candidatePhysicalGeneration=PlanetarySphericalBillboardNaturalTerrainProof.PhysicalGeneration;candidateTerrainGeneration=PlanetarySphericalBillboardNaturalTerrainProof.TerrainDataGeneration;candidatePupilGeneration=pupil.Generation;candidateTopologyHash=topology.TopologyHash;
        Console.WriteLine($"P2S5C1 candidate prepared: interactive=true; level={topology.Level}; hash=0x{topology.TopologyHash:X16}; vertices={topology.Vertices.Count}; triangles={topology.TriangleCount}; physicalGpuMs={physical.Metrics.GpuMilliseconds:F6}; heightParity={physical.MaximumCpuHeightErrorMetres:E9}m; normalParity={physical.MaximumCpuNormalErrorRadians:E9}rad; runtimeTopologyGeneration=0");
    }
    ...
    fixed(NativeRenderObject* po=objects)fixed(NativeDrawBatch* pb=batches)fixed(NativeOrbitLineVertex* pl=orbit)fixed(NativeOrbitLineVertex* pp=previousOrbit)fixed(NativeOrbitLineVertex* pf=bodyForward)fixed(NativeOrbitLineVertex* pt=targetDirection)fixed(NativePlanetaryPatch* px=planetaryPatches)fixed(NativePlanetaryPresentation* pd=distantBodies)fixed(NativeAnchoredSurfacePatch* pas=anchoredSurfacePatches)fixed(NativeProductionBillboardLatticeVertex* pcl=candidateLattice)fixed(uint* pci=candidateIndices)fixed(NativeSphericalBillboardPhysicalVertex* pcp=candidatePhysical)fixed(byte* terrainPath=terrainPathUtf8)fixed(byte* localTerrainPath=localTerrainPathUtf8)fixed(byte* oraclePath=elevationOraclePathUtf8){NativeProductionSphericalBillboardSubmission candidate=new(){Size=(uint)sizeof(NativeProductionSphericalBillboardSubmission),Version=1,Enabled=productionBillboardCandidate?1u:0u,Level=candidateLevel,VertexCount=(uint)candidateLattice.Length,IndexCount=(uint)candidateIndices.Length,LatticeScale=candidateLatticeScale,PhysicalGeneration=candidatePhysicalGeneration,TerrainDataGeneration=candidateTerrainGeneration,PupilGeneration=candidatePupilGeneration,TopologyHash=candidateTopologyHash,PublicationGeneration=1,LatticeVertices=pcl,Indices=pci,PhysicalVertices=pcp};NativeFrameSubmission n=new(){Camera=NativeCamera(state.Submission.Camera),Objects=po,ObjectCount=(uint)state.Submission.ObjectCount,Batches=pb,BatchCount=(uint)batchCount,OrbitVertices=pl,OrbitVertexCount=(uint)(sol?.OrbitVertices.Length??state.Submission.OrbitVertexCount),PreviousOrbitVertices=pp,PreviousOrbitVertexCount=(uint)state.Submission.PreviousOrbitVertexCount,BodyForwardVertices=pf,BodyForwardVertexCount=(uint)state.Submission.BodyForwardVertexCount,TargetDirectionVertices=pt,TargetDirectionVertexCount=(uint)state.Submission.TargetDirectionVertexCount,PlanetaryPatches=px,PlanetaryPatchCount=ActivePlanetaryPatchCount(state),PlanetaryGpu=earth?.GpuConstants(camera)??sol?.GpuConstants(camera)??default,PlanetaryMode=earth?.Mode??(sol is null?NativePlanetaryMode.CpuReference:NativePlanetaryMode.GpuProduction),PlanetarySurfaceMode=NativeSurfaceMode(earth,sol),PhysicalSurfaceGeneration=(uint)options.PhysicalSurfaceGeneration,PlanetaryPresentation=earth?.NativePresentation(camera)??sol?.FocusedPresentation(camera)??default,DistantBodies=pd,DistantBodyCount=(uint)(sol?.DistantBodyCount??0),SolarLighting=earth?.SolarLighting(camera)??sol?.SolarLighting(camera)??default,AnchoredSurfacePatches=pas,AnchoredSurfacePatchCount=(uint)(dynamicAnchored?.ActivePatchCount??0),AnchoredSurfaceCacheSlotCount=(uint)(dynamicAnchored?.CacheCapacity??0),AnchoredSurfaceActiveGeneration=dynamicAnchored?.ActiveGeneration??0u,AnchoredSurfaceFlags=dynamicAnchored?.Visible==true?1u|(dynamicAnchored.RequiresGlobalFallback?2u:0u):0u,AnchoredSurfacePresentation=dynamicAnchored?.NativePresentation??default,ProductionBillboard=productionBillboardCandidate?&candidate:null,ProductionBillboardFlags=productionBillboardCandidate?1u:0u};var runtimeAssets=new NativeRuntimeAssets{Size=(uint)sizeof(NativeRuntimeAssets),Version=3,ProductionTerrainPathUtf8=terrainPath,LocalTerrainPathUtf8=localTerrainPath,ElevationOraclePathUtf8=oraclePath};try{return terrainPath is null?NativeRuntime.RunRenderer(&n,Callback,GCHandle.ToIntPtr(h))==NativeResult.Success?0:1:NativeRuntime.RunRendererWithAssets(&n,Callback,GCHandle.ToIntPtr(h),&runtimeAssets)==NativeResult.Success?0:1;}finally{h.Free();}}
}
