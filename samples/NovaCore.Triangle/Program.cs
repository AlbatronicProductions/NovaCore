...
static unsafe int Run(SampleOptions options,LogOptions log)
{
    ...
    var objects=new NativeRenderObject[snapshot.Count];var batches=new NativeDrawBatch[snapshot.Count];var orbit=new NativeOrbitLineVertex[snapshot.OrbitCurve?.Count??0];var h=GCHandle.Alloc(state);
    CopyObjects(state.Submission,objects,batches,orbit,out var batchCount);
    fixed(NativeRenderObject* po=objects)fixed(NativeDrawBatch* pb=batches)fixed(NativeOrbitLineVertex* pl=orbit){NativeFrameSubmission n=new(){Camera=NativeCamera(state.Submission.Camera),Objects=po,ObjectCount=(uint)objects.Length,Batches=pb,BatchCount=(uint)batchCount,OrbitVertices=pl,OrbitVertexCount=(uint)state.Submission.OrbitVertexCount};try{return NativeRuntime.RunRenderer(&n,Callback,GCHandle.ToIntPtr(h))==NativeResult.Success?0:1;}finally{h.Free();}}
}
...
static unsafe void Callback(NativeHostEvent* e,IntPtr data)
{
    ...
    if(celestialScene is not null){celestialScene.ApplyPresentationInput(s.Camera,e->Input,out var rateChanged,out var pauseChanged);var duration=SimulationDuration.FromSecondsRounded(Math.Clamp((double)e->Input.DeltaSeconds,0d,FreeCameraController.MaximumDeltaSeconds));if(!celestialScene.TryAdvanceByHostDuration(duration,out var publicationError)&&!s.CelestialFailureLogged){Console.Error.WriteLine(publicationError);s.CelestialFailureLogged=true;}else s.Snapshot=celestialScene.CurrentSnapshot;if(s.Log.IsEnabled(LogCategory.Camera)&&rateChanged)Console.WriteLine($"Celestial rate={celestialScene.Rate.Numerator}:{celestialScene.Rate.Denominator}");if(s.Log.IsEnabled(LogCategory.Camera)&&pauseChanged)Console.WriteLine($"Celestial paused={celestialScene.IsPaused}");if(s.Log.IsEnabled(LogCategory.Camera)&&e->Input.MouseWheelDetents!=0)Console.WriteLine($"Celestial zoom distance={celestialScene.OrbitDistance:R}");}
    if(e->Input.Reset!=0){s.Camera.Position=s.Default.Position;s.Camera.Orientation=s.Default.Orientation;s.Camera.Projection=s.Default.Projection;s.Camera.Mode=s.Default.Mode;s.Controller.Reset();celestialScene?.ResetPresentationCamera(s.Camera);}else s.Controller.Update(s.Camera,commands[..count],e->Input.DeltaSeconds);
    if(!TryBuild(s,out var buildError)){Console.Error.WriteLine(buildError);PostQuitMessage(1);return;} CopyObjectsToNative(s.Submission,e->Submission->Objects,e->Submission->Batches,e->Submission->OrbitVertices,out var batchCount);e->Submission->ObjectCount=(uint)s.Submission.ObjectCount;e->Submission->BatchCount=(uint)batchCount;e->Submission->OrbitVertexCount=(uint)s.Submission.OrbitVertexCount;e->Submission->Camera=NativeCamera(s.Submission.Camera);
...
{
    Console.WriteLine("Scene: celestial");Console.WriteLine("Clock-driven analytical two-body marker fixture; no prescribed orbital phase.");Console.WriteLine($"Presentation: 1 display unit = {CelestialAnalyticalScene.MetresPerDisplayUnit:R} m; clock rate={CelestialAnalyticalScene.SampleRate}:1.");Console.WriteLine("Scheduled inertial impulse: +200 m/s tangential at 100000 simulated seconds.");Console.WriteLine("Controls: LMB/RMB drag orbit, wheel zoom, WASD/QE fly, ,/. rate, Space pause, R camera reset.");Console.WriteLine("Objects: central marker + satellite marker; triangle geometry is presentation-only.");
}
[DllImport("user32.dll")] static extern void PostQuitMessage(int exitCode);
static unsafe void CopyObjects(RenderFrameSubmission s,NativeRenderObject[] o,NativeDrawBatch[] b,NativeOrbitLineVertex[] l,out int count){fixed(NativeRenderObject* po=o)fixed(NativeDrawBatch* pb=b)fixed(NativeOrbitLineVertex* pl=l)CopyObjectsToNative(s,po,pb,pl,out count);}
static unsafe void CopyObjectsToNative(RenderFrameSubmission s,NativeRenderObject* o,NativeDrawBatch* b,NativeOrbitLineVertex* l,out int count){for(var i=0;i<s.ObjectCount;i++){var x=s.Objects[i];o[i]=new(){Position=NativePosition(x.Position),Transform=new(){RotationX=x.Transform.Rotation.X,RotationY=x.Transform.Rotation.Y,RotationZ=x.Transform.Rotation.Z,RotationW=x.Transform.Rotation.W,ScaleX=x.Transform.Scale.X,ScaleY=x.Transform.Scale.Y,ScaleZ=x.Transform.Scale.Z},Mesh=new(){Value=x.Mesh.Value}};}count=s.BatchCount;for(var i=0;i<count;i++){var x=s.Batches[i];b[i]=new(){Mesh=new(){Value=x.Mesh.Value},FirstObject=x.FirstObject,ObjectCount=x.ObjectCount};}for(var i=0;i<s.OrbitVertexCount;i++){var x=s.OrbitVertices[i];l[i]=new(){X=x.X,Y=x.Y,Z=x.Z};}}
