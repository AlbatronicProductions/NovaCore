...
static unsafe int Run(SampleOptions options,LogOptions log)
{
    ...
    var state=new HostState(new CameraState(new FramePosition(root,cameraPosition),DoubleQuaternion.Identity,projection,CameraMode.Free),frames,new ReferenceFrameResolver(frames),snapshot!,new RenderFrameSubmission(snapshot!.Count),log,movementSpeed,dynamic,celestial);
    if(!TryBuild(state,out var buildError)){Console.Error.WriteLine(buildError);return 1;}
    var objects=new NativeRenderObject[snapshot.Count];var batches=new NativeDrawBatch[snapshot.Count];var h=GCHandle.Alloc(state);
    CopyObjects(state.Submission,objects,batches,out var batchCount);
...
static unsafe void Callback(NativeHostEvent* e,IntPtr data)
{
    ...
    if(!TryBuild(s,out var buildError)){Console.Error.WriteLine(buildError);PostQuitMessage(1);return;} CopyObjectsToNative(s.Submission,e->Submission->Objects,e->Submission->Batches,out var batchCount);e->Submission->ObjectCount=(uint)s.Submission.ObjectCount;e->Submission->BatchCount=(uint)batchCount;e->Submission->Camera=NativeCamera(s.Submission.Camera);
...
static unsafe void CopyObjects(RenderFrameSubmission s,NativeRenderObject[] o,NativeDrawBatch[] b,out int count){fixed(NativeRenderObject* po=o)fixed(NativeDrawBatch* pb=b)CopyObjectsToNative(s,po,pb,out count);}
static unsafe void CopyObjectsToNative(RenderFrameSubmission s,NativeRenderObject* o,NativeDrawBatch* b,out int count){for(var i=0;i<s.ObjectCount;i++){var x=s.Objects[i];o[i]=new(){Position=NativePosition(x.Position),Transform=new(){RotationX=x.Transform.Rotation.X,RotationY=x.Transform.Rotation.Y,RotationZ=x.Transform.Rotation.Z,RotationW=x.Transform.Rotation.W,ScaleX=x.Transform.Scale.X,ScaleY=x.Transform.Scale.Y,ScaleZ=x.Transform.Scale.Z},Mesh=new(){Value=x.Mesh.Value}};}count=s.BatchCount;for(var i=0;i<count;i++){var x=s.Batches[i];b[i]=new(){Mesh=new(){Value=x.Mesh.Value},FirstObject=x.FirstObject,ObjectCount=x.ObjectCount};}}
