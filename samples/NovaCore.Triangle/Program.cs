...
var productionEarth=args.Contains("--scene=m12d-production-spherical-billboard",StringComparer.OrdinalIgnoreCase);
if(productionEarth)
{
...
if(!SampleOptions.TryParse(args,out var options,out var error)){Console.Error.WriteLine(error);return 2;}if(!LogOptions.TryParse(options.LogArguments,out var log,out var logError)){Console.Error.WriteLine(logError);return 2;}return Run(options,log,options.UseProductionEarth);
...
static unsafe int Run(SampleOptions options,LogOptions log,bool productionEarth)
{
    PlanetaryPhysicalSurface.ConfigureRuntimeGeneration(options.PhysicalSurfaceGeneration);
    if(options.Scene is "earth" or "sol")Console.WriteLine($"Earth route policy: scene={options.Scene}; owner=NCSM1; physicalGeneration={(uint)options.PhysicalSurfaceGeneration}; focus={options.InitialFocus}; altitude={options.AltitudeMetres}; site={options.SurfaceSite}; anchoredInstantiated=false; manualAcceptance=passed.");
    ...
    string? terrainAssetPath=null,localTerrainAssetPath=null,elevationOraclePath=null,terrainAssetSha=null,localTerrainAssetSha=null;if(options.UseProductionEarth)
    {
    ...
    ResolvedRenderSnapshot? snapshot=null; FixtureSceneDiagnostics fixture=default; DynamicReferenceFrameFixtureScene? dynamic=null; CelestialAnalyticalScene? celestial=null; EarthPlanetaryScene? earth=null; SolarSystemScene? sol=null; ContactDevelopmentScene? contact=null; PoweredFlightDevelopmentScene? powered=null; PoweredContactDevelopmentScene? poweredContact=null; StockAssemblyDevelopmentScene? assembly=null; var cameraPosition=defaultStart.Value;var cameraOrientation=DoubleQuaternion.Identity; var projection=new CameraProjection(Math.PI/3,16d/9,.01,1000); var movementSpeed=.1d;
    if(options.Scene is "stock-assembly" or "srv01-supported-contact" or "srv01-powered-support" or "srv01-florida-support")
    {
        assembly=new(options.StockDesign,options.Scene!="stock-assembly",options.Scene=="srv01-powered-support",options.Scene=="srv01-florida-support");snapshot=assembly.InitialSnapshot;cameraPosition=new(2,3,8);
        cameraOrientation=DoubleQuaternion.FromAxisAngle(Double3.UnitX,-Math.Atan2(3,8));movementSpeed=2;
        sol=assembly.FloridaView?.Solar;
    }
    else if(options.Scene=="powered-contact")
    {
        poweredContact=new();snapshot=poweredContact.InitialSnapshot;cameraPosition=new(6,4,8);
        cameraOrientation=DoubleQuaternion.FromAxisAngle(Double3.UnitY,Math.Atan2(6,8))*DoubleQuaternion.FromAxisAngle(Double3.UnitX,-Math.Atan2(3,10));movementSpeed=3;
    }
    else if(options.Scene=="powered-free-flight")
    {
        powered=new();snapshot=powered.InitialSnapshot;cameraPosition=new(14,8,30);
        cameraOrientation=DoubleQuaternion.FromAxisAngle(Double3.UnitX,-Math.Atan2(8,30));movementSpeed=5;
    }
    else if(options.Scene is "contact-centered" or "contact-tilted" or "article-centered" or "article-tilted")
    {
    ...
    else if(options.Scene=="sol")
    {
        ...
        if(options.PlayerFlightControls)assembly=new(options.StockDesign,solarWorld:sol,playerControls:true);else if(options.UsesFloridaSlab)assembly=new(options.StockDesign,supportedContact:true,floridaSupport:true,solarWorld:sol);
    ...
    if(assembly?.FloridaView is { } floridaView)floridaView.PrepareCamera(camera);else sol?.ResetPresentationCamera(camera);
    if(sol is not null&&options.SurfaceSite=="florida-launch"&&assembly is null)
    {
        if(!sol.TryStartAtFloridaLaunchSite(camera)){Console.Error.WriteLine("Florida launch-site camera startup failed.");return 1;}
    }
    else if(sol is not null&&options.InitialFocus!=NativePresentationFocus.None&&assembly is null)
    {
    ...
    if(options.UseProductionEarth)PrintRuntimeFingerprint(terrainAssetPath,terrainAssetSha,localTerrainAssetPath,localTerrainAssetSha,elevationOraclePath);
    if(sol is not null && (options.SurfaceSite=="florida-launch"||assembly?.FloridaView is not null))
    {
        var site=sol.FloridaLaunchSite;var direction=site.Object.Anchor.NormalizedBodyFixedDirection;
        Console.WriteLine($"Florida seating: body={site.Object.Anchor.BodyId}; id=0x{site.Object.Id.Value:X16}; direction=({direction.X:R},{direction.Y:R},{direction.Z:R}); physicalGeneration={(uint)PlanetaryPhysicalSurface.RuntimeGeneration}; terrainVersion={site.Object.Anchor.TerrainAuthorityVersion.Version}; H={site.AnchorTerrainHeightMetres:R}; slabRootHeight={site.AnchorTerrainHeightMetres+site.FoundationOffsetMetres:R}; foundationDepth={site.FoundationDepthMetres:R}; foundationBottomLocalZ={-site.FoundationDepthMetres:R}; foundationBottomCenterHeight={site.AnchorTerrainHeightMetres+site.FoundationOffsetMetres-site.FoundationDepthMetres:R}; minimumContactLocalZ={site.MinimumTerrainLocalUpMetres:R}; embed={FloridaLaunchSite.FoundationEmbedMetres:R}; regionalLoaded={EarthLocalTerrainElevationDataset.IsLoaded}; regionalResidual={EarthLocalTerrainElevationDataset.SampleResidual(direction):R}; regionalSha={localTerrainAssetSha}; authority=canonical-H; publicationDependent=false.");
    }
    if(sol is not null&&assembly?.UsesSolarCamera==true)
    {
        if(!sol.BindActiveVessel(assembly.PrepareFocusObservation())||!sol.RefocusActiveVessel(camera))
            throw new InvalidOperationException("Active spacecraft camera binding refused.");
        Console.WriteLine("Camera: F focuses SRV-01; mouse drag orbits; wheel zooms; 1-0 focus celestial bodies; R restores Solar overview. Free camera is deferred; Home is unbound. Camera actions do not pilot the craft.");
    }
    var objectCapacity=assembly?.RenderCapacity??(snapshot!.Count+(sol is null?0:2));var state=new HostState(camera,frames,new ReferenceFrameResolver(frames),snapshot!,new RenderFrameSubmission(objectCapacity,Math.Max(snapshot!.OrbitCurve?.Count??0,sol?.OrbitVertices.Length??0)),log,movementSpeed,dynamic,celestial,earth,planetaryPatches,sol,options.BenchmarkFrames,options.SolarWarpTraversal);state.Contact=contact;state.Powered=powered;state.PoweredContact=poweredContact;state.Assembly=assembly;state.AssemblyAutoStart=!options.PlayerFlightControls&&(options.BenchmarkFrames>0||assembly?.FloridaView?.SolarNavigation==true);
    sol?.EnforceFinalCameraInvariant(camera);
    UpdatePlanetaryPatches(state);
    if(!TryBuild(state,out var buildError)){Console.Error.WriteLine(buildError);contact?.Dispose();poweredContact?.Dispose();assembly?.Dispose();return 1;}
    if(productionEarth)
    {
        ...
        if(options.ProductionBillboardDesktopTraversal)
        {
            ...
            state.ProductionBillboardDesktopTraversal.ApplyPose(camera,sol!);
            sol!.EnforceFinalCameraInvariant(camera);
            if(!TryBuild(state,out buildError)){Console.Error.WriteLine(buildError);return 1;}
        }
    ...
    var facilityCaster=assembly?.FloridaView is not null?default:sol?.FacilityCaster??default; // No hidden cube/arm caster in the single-slab route.
    fixed(NativeRenderObject* po=objects)fixed(NativeDrawBatch* pb=batches)fixed(NativeOrbitLineVertex* pl=orbit)fixed(NativeOrbitLineVertex* pp=previousOrbit)fixed(NativeOrbitLineVertex* pf=bodyForward)fixed(NativeOrbitLineVertex* pt=targetDirection)fixed(NativePlanetaryPatch* px=planetaryPatches)fixed(NativePlanetaryPresentation* pd=distantBodies)fixed(byte* terrainPath=terrainPathUtf8)fixed(byte* localTerrainPath=localTerrainPathUtf8)fixed(byte* oraclePath=elevationOraclePathUtf8){var productionBillboardPresentation=productionEarth&&ProductionBillboardPresentationEligible(state);NativeFrameSubmission n=new(){FacilityCaster=facilityCaster.BodyId==0?null:&facilityCaster,Camera=NativeCamera(state.Submission.Camera),Objects=po,ObjectCount=(uint)state.Submission.ObjectCount,Batches=pb,BatchCount=(uint)batchCount,OrbitVertices=pl,OrbitVertexCount=(uint)(sol?.OrbitVertices.Length??state.Submission.OrbitVertexCount),PreviousOrbitVertices=pp,PreviousOrbitVertexCount=(uint)state.Submission.PreviousOrbitVertexCount,BodyForwardVertices=pf,BodyForwardVertexCount=(uint)state.Submission.BodyForwardVertexCount,TargetDirectionVertices=pt,TargetDirectionVertexCount=(uint)state.Submission.TargetDirectionVertexCount,PlanetaryPatches=px,PlanetaryPatchCount=ActivePlanetaryPatchCount(state),PlanetaryGpu=earth?.GpuConstants(camera)??sol?.GpuConstants(camera)??default,PlanetaryMode=earth?.Mode??(sol is null?NativePlanetaryMode.CpuReference:NativePlanetaryMode.GpuProduction),PlanetarySurfaceMode=NativeSurfaceMode(earth,sol),PhysicalSurfaceGeneration=(uint)options.PhysicalSurfaceGeneration,PlanetaryPresentation=earth?.NativePresentation(camera)??sol?.FocusedPresentation(camera)??default,DistantBodies=pd,DistantBodyCount=(uint)(sol?.DistantBodyCount??0),SolarLighting=earth?.SolarLighting(camera)??sol?.SolarLighting(camera)??default,ProductionBillboard=productionBillboardPresentation?state.ProductionBillboardLease?.Pointer:null,ProductionBillboardFlags=productionBillboardPresentation?1u:0u,ProductionBillboardFrame=productionBillboardPresentation?state.ProductionBillboardFrameLease?.Pointer:null};var runtimeAssets=new NativeRuntimeAssets{Size=(uint)sizeof(NativeRuntimeAssets),Version=3,ProductionTerrainPathUtf8=terrainPath,LocalTerrainPathUtf8=localTerrainPath,ElevationOraclePathUtf8=oraclePath};try{if(assembly is not null)return (sol is not null?assembly.Visuals.Run(&n,Callback,GCHandle.ToIntPtr(h),(uint)assembly.RenderCapacity,&runtimeAssets):assembly.Visuals.Run(&n,Callback,GCHandle.ToIntPtr(h),(uint)assembly.RenderCapacity))==NativeResult.Success?0:1;return terrainPath is null?NativeRuntime.RunRenderer(&n,Callback,GCHandle.ToIntPtr(h))==NativeResult.Success?0:1:NativeRuntime.RunRendererWithAssets(&n,Callback,GCHandle.ToIntPtr(h),&runtimeAssets)==NativeResult.Success?0:1;}finally{state.Assembly?.Dispose();state.Contact?.Dispose();state.PoweredContact?.Dispose();state.DisposeProductionBillboardLeases();h.Free();}}
}
...
static bool TryBuild(HostState s,out string error)
{
    ...
    if(s.Assembly is { } assembly){assembly.BuildSubmission(camera,rootPosition,s.Submission);error=string.Empty;return true;}
    if(s.PoweredContact is { } poweredContact){poweredContact.BuildSubmission(camera,rootPosition,s.Submission);error=string.Empty;return true;}
    if(s.Powered is { } powered){powered.BuildSubmission(camera,rootPosition,s.Submission);error=string.Empty;return true;}
    if(s.Contact is { } contact){contact.BuildSubmission(camera,rootPosition,s.Submission);error=string.Empty;return true;}
    ...
    if(s.Sol?.TryGetFloridaLaunchSitePresentation(s.Camera,out var sitePosition,out var siteOrientation)==true)
    {
        ...
        s.Submission.Add(sitePosition,siteOrientation,s.Sol.FloridaLaunchSite.FoundationScale,MeshHandle.FloridaLaunchFoundation);
...
static unsafe void Callback(NativeHostEvent* e,IntPtr data)
{
    ...
    s.Powered?.AdvanceLive(e->Input.PauseToggle!=0);
    s.Assembly?.ApplyPlayerInput(e->Input);
    s.Assembly?.AdvanceLive(s.Assembly?.FloridaView?.SolarNavigation==true?s.AssemblyAutoStart:e->Input.PauseToggle!=0||s.AssemblyAutoStart);s.AssemblyAutoStart=false;
    s.PoweredContact?.AdvanceLive(e->Input.PauseToggle!=0);
    var celestialScene=s.Celestial;var earthScene=s.Earth;var orbitCamera=celestialScene is not null||earthScene is not null||s.Sol is not null&&(s.Assembly?.FloridaView is null||s.Assembly.FloridaView.SolarNavigation);Span<CameraCommand> commands=stackalloc CameraCommand[4];var count=DebugCameraInput.Map(s.Assembly?.PlayerInput is not null?PlayerFlightControlInput.CameraInput(e->Input):e->Input,commands,!orbitCamera,!orbitCamera);var previousSpeed=s.Controller.MovementSpeed;
    ...
    if(s.Sol is not null&&(s.Assembly?.FloridaView is null||s.Assembly.FloridaView.SolarNavigation)){var solarScene=s.Sol;var solarInput=s.ProductionBillboardDesktopTraversal?.PrepareInput(e->Input,solarScene)??s.SolarWarpTraversal?.PrepareInput(e->Input,solarScene)??e->Input;if(s.Assembly?.PlayerInput is not null)solarInput=PlayerFlightControlInput.CameraInput(solarInput);solarScene.ApplyPresentationInput(s.Camera,solarInput,out var rateChanged,out var pauseChanged,deferVesselPose:true);var duration=SimulationDuration.FromSecondsRounded(Math.Clamp((double)solarInput.DeltaSeconds,0d,FreeCameraController.MaximumDeltaSeconds));if(!solarScene.TryAdvanceByHostDuration(duration,s.Camera,out var publicationError)&&!s.SolarFailureLogged){Console.Error.WriteLine(publicationError);s.SolarFailureLogged=true;}if(s.Log.IsEnabled(LogCategory.Camera)&&rateChanged)Console.WriteLine($"Solar rate={solarScene.Rate.Numerator}:{solarScene.Rate.Denominator}");if(s.Log.IsEnabled(LogCategory.Camera)&&pauseChanged)Console.WriteLine($"Solar paused={solarScene.IsPaused}");if(s.Log.IsEnabled(LogCategory.Camera)&&solarInput.MouseWheelDetents!=0)Console.WriteLine($"Solar orbit distance={solarScene.OrbitDistance:R} m");if(solarScene.Focus(s.Camera,solarInput.PresentationFocus))Console.WriteLine($"Solar focus: {solarScene.FocusedBody.Label}; id={solarScene.FocusedBody.BodyId}; radius={solarScene.FocusedBody.RadiusMetres:R} m; distance/radius={solarScene.FocusedBlend.DistanceRadii:R}; state={solarScene.FocusedBlend.Regime}");LogSolarTelemetry(s,solarScene);}
    var resetRequested=s.ProductionBillboardDesktopTraversal is null&&e->Input.Reset!=0;
    if(resetRequested){s.Camera.Position=s.Default.Position;s.Camera.Orientation=s.Default.Orientation;s.Camera.Projection=s.Default.Projection;s.Camera.Mode=s.Default.Mode;s.Controller.Reset();celestialScene?.ResetPresentationCamera(s.Camera);earthScene?.ResetPresentationCamera(s.Camera);if(s.Assembly?.FloridaView is null||s.Assembly.FloridaView.SolarNavigation)s.Sol?.ResetPresentationCamera(s.Camera);}else if(!orbitCamera)s.Controller.Update(s.Camera,commands[..count],e->Input.DeltaSeconds);
    // Freeze the copied material-origin/display mapping after Solar publication,
    // including paused/held endpoints, before camera-relative submission.
    if(s.Sol is { } vesselSolar&&s.Assembly?.UsesSolarCamera==true)
        vesselSolar.RefreshActiveVessel(s.Camera,s.Assembly.PrepareFocusObservation());
    ApplyViewport(s,e->Input);
    s.Sol?.EnforceFinalCameraInvariant(s.Camera);
    if(s.Sol is { } c3Solar)s.ProductionBillboardDesktopTraversal?.ApplyPose(s.Camera,c3Solar);
    if(s.Sol is { } routeSolar)s.RouteValidation?.Advance(routeSolar,s.Camera);
...
static unsafe void UpdateProductionBillboard(HostState state,NativeFrameSubmission* submission)
{
    ...
    var hasPrepared=state.ProductionBillboardLease is null&&runtime.TrySubmitPrepared(out prepared);
    if(state.ProductionBillboardLease is null&&runtime.Current is null&&!hasPrepared)
    {
        while(!runtime.TrySubmitPrepared(out prepared))Thread.Sleep(1);
        hasPrepared=true;
    }
    if(hasPrepared)
    {
    ...
    if(state.ProductionBillboardDesktopTraversal is { } traversal&&state.Sol is { } solar&&
       traversal.Observe(runtime,telemetry,solar))
    {
