...
static unsafe int Run(SampleOptions options,LogOptions log,bool productionEarth)
{
    ...
    var objectCapacity=snapshot!.Count+(sol is null?0:2);var state=new HostState(camera,frames,new ReferenceFrameResolver(frames),snapshot!,new RenderFrameSubmission(objectCapacity,Math.Max(snapshot.OrbitCurve?.Count??0,sol?.OrbitVertices.Length??0)),log,movementSpeed,dynamic,celestial,earth,planetaryPatches,sol,options.BenchmarkFrames,options.SolarWarpTraversal);state.Contact=contact;state.Powered=powered;state.PoweredContact=poweredContact;
    sol?.EnforceFinalCameraInvariant(camera);
    UpdatePlanetaryPatches(state);
    if(!TryBuild(state,out var buildError)){Console.Error.WriteLine(buildError);contact?.Dispose();poweredContact?.Dispose();return 1;}
    if(productionEarth)
    {
