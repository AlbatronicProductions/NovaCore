...
static unsafe int Run(SampleOptions options,LogOptions log)
{
    ...
    if(options.Scene=="planetary-diagnostic")
    {
    ...
    var planetaryPatches=options.Scene=="planetary-diagnostic"?CreateDiagnosticPatches():[];
    var state=new HostState(new CameraState(new FramePosition(root,cameraPosition),DoubleQuaternion.Identity,projection,CameraMode.Free),frames,new ReferenceFrameResolver(frames),snapshot!,new RenderFrameSubmission(snapshot!.Count,snapshot.OrbitCurve?.Count??0),log,movementSpeed,dynamic,celestial,planetaryPatches);
    UpdateDiagnosticPatches(state);
...
static void UpdateDiagnosticPatches(HostState state){if(state.PlanetaryPatches.Length==0)return;var relative=Double3.Zero-state.Camera.Position.Value;foreach(ref var patch in state.PlanetaryPatches.AsSpan()){patch.CenterX=(float)relative.X;patch.CenterY=(float)relative.Y;patch.CenterZ=(float)relative.Z;}}
file sealed class HostState(CameraState camera,ReferenceFrameSnapshot frames,ReferenceFrameResolver resolver,ResolvedRenderSnapshot snapshot,RenderFrameSubmission submission,LogOptions log,double movementSpeed,DynamicReferenceFrameFixtureScene? dynamic,CelestialAnalyticalScene? celestial,NativePlanetaryPatch[] planetaryPatches){public CameraState Camera=camera;public CameraState Default=new(new FramePosition(camera.Position.Frame,camera.Position.Value),camera.Orientation,camera.Projection,camera.Mode);public ReferenceFrameSnapshot Frames=frames;public ReferenceFrameResolver Resolver=resolver;public ResolvedRenderSnapshot Snapshot=snapshot;public RenderFrameSubmission Submission=submission;public LogOptions Log=log;public FreeCameraController Controller=new(movementSpeed,.002,Math.PI*89/180);public DynamicReferenceFrameFixtureScene? Dynamic=dynamic;public CelestialAnalyticalScene? Celestial=celestial;public NativePlanetaryPatch[] PlanetaryPatches=planetaryPatches;public bool DynamicFailureLogged;public bool CelestialFailureLogged;public bool HasSasIndicatorDiagnostic;public bool LastSasTargetValid;public long NextSasIndicatorDiagnosticTimestamp;}
file readonly record struct SampleOptions(int ObjectCount,string Scene,string[] LogArguments){public static bool TryParse(string[] a,out SampleOptions v,out string? e){var n=100;var scene="grid";var logs=new List<string>();foreach(var x in a){if(x.StartsWith("--objects=")&&(!int.TryParse(x[10..],out n)||n is not(1 or 100 or 1000 or 10000))){v=default;e="Usage: --objects=1|100|1000|10000";return false;}if(x.StartsWith("--scene="))scene=x[8..];else logs.Add(x);}if(scene is not("grid"or"frames"or"fixture"or"fixture-dynamic"or"celestial"or"planetary-diagnostic")){v=default;e="Usage: --scene=grid|frames|fixture|fixture-dynamic|celestial|planetary-diagnostic";return false;}v=new(n,scene,logs.ToArray());e=null;return true;}}
