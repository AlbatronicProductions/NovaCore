...
The current celestial fixture uses an intentionally mildly overdamped SAS configuration. Its `(120, 120, 120) kg·m²` inertia and `(8, 8, 8) N·m` available SAS torque are fixture-specific rather than engine-wide response targets. The deterministic convergence checks accept 90-degree Prograde, Normal, and Radial Out acquisition within 35 simulated seconds and 180-degree Retrograde within 55 seconds, while retaining `.01` radian attitude/rate settlement thresholds. The objective is stable, non-snapping demonstration behavior; different spacecraft will require their own inertia, actuator, and response definitions.
