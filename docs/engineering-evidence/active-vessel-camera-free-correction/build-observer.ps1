param([ValidateSet('candidate')][string]$Version='candidate')
# Baseline remains the retained HEAD observer from the previous camera package.
# Reproducible disposable compile overlay. Production source is never rewritten.
$ErrorActionPreference='Stop'
$repo=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
Set-Location -LiteralPath $repo
$output=Join-Path $repo "build/active-vessel-camera-free-correction/observer-$Version"
...
dotnet build samples/NovaCore.Triangle/NovaCore.Triangle.csproj -c Release --artifacts-path "build/active-vessel-camera-free-correction/$Version" -o "$output/runtime" -p:BuildProjectReferences=false "-p:CustomAfterMicrosoftCommonTargets=$output/observer.targets" "-p:NativeBuildDirectory=active-vessel-camera-free-correction/$native" --nologo -v:q *> "$output/build.txt"
...
Get-ChildItem "build/active-vessel-camera-free-correction/$Version/bin/NovaCore.Triangle/release/NovaCore.*.dll" |
