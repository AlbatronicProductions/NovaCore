param([Parameter(Mandatory)][ValidateSet('A1','B1','B2','A2')][string]$Id)
...
$record=[ordered]@{id=$Id;order=$position+1;route=$route;start=(Get-Date).ToString('o');arguments=@("--scene=$route",'--benchmark-frames=180');executableSha256=(Get-FileHash $exe).Hash;interaction='none intentionally';freshProcess=$true}
