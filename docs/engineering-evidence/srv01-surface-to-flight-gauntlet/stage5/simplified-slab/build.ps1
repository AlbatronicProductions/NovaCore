...
$output='build/srv01-stage5-simplified-slab'
...
dotnet build NovaCore.sln -c $Configuration --artifacts-path "$output/artifacts" -p:ContinuousIntegrationBuild=true "-p:NativeBuildDirectory=srv01-stage5-simplified-slab/native-$suffix" --nologo -v:q *> "$output/full-build-$suffix.txt"
