"""One reusable capture slot; placement A/B, one baseline repeat only on mismatch.

Run by the lead only after whole.py build-v3. No build, deployment or cleanup.
python -B placement_parity.py florida placement-florida
Capture bytes are not performance measurements. The three raw files remain in
whole.OUT for the lead's final bounded cleanup; no duplicate archive is created.
"""
import argparse
import hashlib
import json
...
def run_capture(pose, label, local):
    # Enable raw input correlation and disable physical mouse/wheel injection.
    # The fixed route still owns the pose; no regional route is selected here.
    original_environment = whole.assess.environment
    def environment():
        env = original_environment()
        env['NOVACORE_COMPOSITION_CONTROL'] = '1'
        return env
    paths = {name: (whole.OUT / name).resolve() for name in NAMES}
    expected_root = whole.OUT.resolve()
    assert all(path.parent == expected_root for path in paths.values())
    before_mtimes = {name: path.stat().st_mtime_ns if path.exists() else None
                     for name, path in paths.items()}
    whole.assess.environment = environment
    started_ns = time.time_ns()
    try:
        result = whole.fixed(pose, label, local=local)
    finally:
        whole.assess.environment = original_environment
    ...
    report = {'label': label, 'local': local, 'capture': record,
              ...
              'runJournalSha256': sha(whole.HERE / (label + '.json')),
              'heaps': [line for line in result['lines'] if 'Whole heap:' in line]}
...
def compare(a, ab, b, bb):
    ...
    # These two intended process-local changes do not describe different poses.
    def comparable_env(r):
        return {k: v for k, v in r['runtime']['env'].items()
                if k not in ('NOVACORE_WHOLE_BULK_LOCAL', 'NOVACORE_PIXEL_PARITY')}
...
def run(pose, label):
    ...
    assert 'noise' not in label and 'physical' not in label
    output = whole.HERE / (label + '-parity.json')
    assert not output.exists(), 'refusing to overwrite retained evidence'
    names = [label + '-capture-' + suffix for suffix in ('base', 'local', 'aa')]
    assert all(not (whole.HERE / (name + '.json')).exists() for name in names)
    ...
    report = {'schema': 1, 'pose': pose, 'frame': FRAME, 'width': WIDTH, 'height': HEIGHT,
              'baseline': a, 'local': b, 'ab': ab, 'pass': ab['pass'],
              'baselineRepeatRun': False, 'rawCaptureSlot': str(whole.OUT.resolve()),
    ...
    if not ab['pass']:
        ...
        report.update(baselineRepeatRun=True, baselineRepeat=aa,
                      aa=compare(a, ap, aa, aap), repeatBaselineVsLocal=compare(aa, aap, b, bp))
        report['pass'] = False
...
if __name__ == '__main__':
    ...
    parser.add_argument('pose', choices=('florida', 'inland', 'active', 'grazing'))
    parser.add_argument('label')
    options = parser.parse_args()
    run(options.pose, options.label)
