...
if __name__=='__main__':
    ...
    elif command=='resume-after-probe-revision':
        env=v.canonical();env['NOVACORE_P2S5F_ARTIFACT_INPUT']=str(ROOT/'assets/planetary-nested-scale-mesh')
        exe=ROOT/'tests/NovaCore.Graphics.Tests/bin/Debug/net10.0/NovaCore.Graphics.Tests.exe'
        v.execute('Debug-window',[exe,'--category=window'],env,900)
        v.execute('Debug-native-gpu',[exe,'--native-gpu'],env)
        v.main('resume-tests')
    elif command=='final-probe-checks':
        for config in ['Debug','Release']:
            exe=ROOT/f'tests/NovaCore.Graphics.Tests/bin/{config}/net10.0/NovaCore.Graphics.Tests.exe'
            v.execute(config+'-final-live-probe',[exe,'--case=Live NCSM1 regional physical residency'],v.canonical(),700)
    else:v.main(command)
