...
internal static partial class Qualification
{
        ...
        CacheDuration nextH,S source,double mass,Load previous,Load next)
    {
        ...
        Save(name+".json",new{status=comparison.Error.Pass?"PASS":"FAIL",old,current,oldH,nextH,source,mass,previous,next,proof,
            target=proposed.Target,comparison.Error,comparison.Reference,acceptedUnchanged=owner.Snapshot==before,sweeps=12});
    ...
    private static void Regressions()
    {
        ...
        RegressionCase("07-composition",g,g,cache,Binary(f.GetProperty("PreviousH").GetDouble()),
        ...
        Save("regression-results.json",new{status="PASS",cases=8,sweeps=12,physicalBar=1e-4,transportBar=1e-12,productionMutations=0});
    }
