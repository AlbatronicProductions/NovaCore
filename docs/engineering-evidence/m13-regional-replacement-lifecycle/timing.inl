...
void ResidualState(App& a){char row[256];std::snprintf(row,sizeof row,"Lifecycle record: frame=%llu; reference=%u; wallMs=%.6f",(unsigned long long)a.frame,lifecycleReferenceTick?1u:0u,LifeNow());a.Log(NC_LOG_ALWAYS,row);}
