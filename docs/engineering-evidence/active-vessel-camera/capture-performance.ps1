param([int]$Repetitions=2,[ValidateSet('live','fixed')][string]$Cadence='live')
$ErrorActionPreference='Stop'
$repo=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
Set-Location -LiteralPath $repo
$output=Join-Path $repo 'build/active-vessel-camera/performance'
...
for($run=1;$run -le $Repetitions;$run++) {
    foreach($route in @(@('baseline','matched'),@('candidate','matched'),@('candidate','new'))) {
        ...
        $exe=Join-Path $repo "build/active-vessel-camera/observer-$version/runtime/NovaCore.Triangle.exe"
