...
[pscustomobject]@{ environmentSha256=$environmentHash; runtimeSettings=$runtimeSettings; dotnetPath=(Get-Command dotnet).Source; workingDirectory='E:\NovaCore'; plan='Debug then Release; revised typed-arena candidate; exactly 3 fresh processes per configuration; no profiler, no GC/tiering changes' } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $comparisonRoot 'environment.json') -Encoding utf8
...
foreach ($configuration in @('Debug','Release')) {
    for ($run = 1; $run -le 3; $run++) {
        foreach ($variant in @('control')) {
            ...
            if ($observation.eventRecordBytes -ne 112 -or !$observation.timelineInvariants -or $observation.pendingEvents -ne 100 -or $observation.completedCancellations -ne 9900 -or $observation.expectedCancellations -ne 9900) { throw "CONTROL INVALID - STOP" }
