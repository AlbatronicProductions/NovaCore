...
static void describe(LONG kind, UINT_PTR id);
static ULONG g_stackCount=0;
static HRESULT __stdcall stackFrame(FunctionID functionId, UINT_PTR ip, COR_PRF_FRAME_INFO, ULONG32, BYTE*, void* client) {
 if(g_stackCount++<16) { record(8,functionId,(UINT_PTR)client); if(functionId)describe(8,functionId); } return S_OK;
}
static void describe(LONG kind, UINT_PTR id) {
 ModuleID module=0; mdTypeDef token=0; ClassID parent=0;
 if(kind==1) { HRESULT hr=g_info->GetClassIDInfo((ClassID)id,&module,&token); if(FAILED(hr)){printf("NAME_FAILURE class=%llu hr=%lx\n",(unsigned long long)id,hr);return;} }
 else { HRESULT hr=g_info->GetFunctionInfo((FunctionID)id,&parent,&module,&token); if(FAILED(hr)){printf("NAME_FAILURE method=%llu hr=%lx\n",(unsigned long long)id,hr);return;} }
 IMetaDataImport* meta=nullptr;
 HRESULT hr=g_info->GetModuleMetaData(module,ofRead,IID_IMetaDataImport,(IUnknown**)&meta); if(FAILED(hr)){printf("META_FAILURE kind=%ld; id=%llu; module=%llu; token=%u; hr=%lx\n",kind,(unsigned long long)id,(unsigned long long)module,token,hr);return;}
 WCHAR name[512]={}; ULONG size=0;
 if(kind==1) meta->GetTypeDefProps(token,name,512,&size,nullptr,nullptr);
 else meta->GetMethodProps(token,nullptr,name,512,&size,nullptr,nullptr,nullptr,nullptr,nullptr);
 printf("ATTRIBUTION_NAME kind=%ld; id=%llu; token=%u; name=%ls\n",kind,(unsigned long long)id,token,name); meta->Release();
}
...
HRESULT STDMETHODCALLTYPE ObjectAllocated(/* [in] */ ObjectID objectId,
            /* [in] */ ClassID classId) override { if (g_active && GetCurrentThreadId() == g_owner) { ULONG bytes=0; g_info->GetObjectSize(objectId, &bytes); ++g_objects; g_objectBytes += bytes; record(1, classId, bytes); describe(1,classId); g_stackCount=0; HRESULT stackResult=g_info->DoStackSnapshot(0,stackFrame,COR_PRF_SNAPSHOT_DEFAULT,(void*)(UINT_PTR)g_objects,nullptr,0); record(9,stackResult,g_objects); } return S_OK; }
