...
from gauntlet import ROOT,HERE,OUT,assess
...
assert OUT.resolve()==ROOT/'build/m13-regional-preparation-blocker'
files=[]
...
manifest=dict(root=str(OUT),files=files,logicalBytes=sum(f['bytes'] for f in files),exclusiveLogicalBytes=sum(f['bytes'] for f in files if f['links']==1),allGitIgnored=True,
 reason='Only private diagnostic host, copied shaders, generated probe shaders, compiler ISA, loader manifest and rejected prototype log. All source/reproduction, compact numeric rows, rejected VUID and hashes retained. No production or permanent regression consumer. Elevation is a hard link; delete only private alias, preserving production asset.',protectedAssets=current['assets'])
