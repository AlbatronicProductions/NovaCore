...
def run(pose,probe,label,capture='none',callback=None,diagnostic=None,capture_frame=175):
    ...
    assert all(sha(DEPLOYED/'shaders'/name)==value for name,value in expected.items() if name!='planetary_production.frag.spv')
    with tempfile.TemporaryDirectory(prefix='isolated-',dir=OUT) as tmp:
        ...
        if probe=='baseline':
            shutil.copy2(OUT/'baseline-native.dll',runtime/'NovaCore.Native.dll')
            shutil.copy2(OUT/'baseline-fragment.spv',runtime/'shaders/planetary_production.frag.spv')
        ...
        # Negative control is private: deliberately specialize a diagnostic as ordinary.
        if probe=='negative':
            fragment=fragment.replace('layout(constant_id=0) const bool ordinaryNcsm1=false;','const bool ordinaryNcsm1=true;')
            compile('planetary_production.frag',fragment)
        if capture=='tes':compile('production_spherical_billboard.tese',shaders.tessellation((SOURCE/'production_spherical_billboard.tese').read_text()))
        ...
        result.update(diagnostic=diagnostic,pipelineIdentity=[x for x in log.splitlines() if 'specialization' in x],fragmentSha256=sha(runtime/'shaders/planetary_production.frag.spv'),pose=pose,probe=probe,captureMode=capture,classification='OUTPUT-ALTERING NEGATIVE CONTROL' if probe=='negative' else 'OUTPUT-PRESERVING INTENT; parity must be checked',shaderVariants=identity,nativeSha256=sha(runtime/'NovaCore.Native.dll'),managedSha256=sha(runtime/'NovaCore.Triangle.dll'),arguments=args,environment={k:v for k,v in env.items() if k.startswith('NOVACORE_')},amdStatistics=[x for x in log.splitlines() if 'AMD shader statistics:' in x],captureIdentity=[x for x in log.splitlines() if 'TES parity capture:' in x])
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('pose',choices=list('ABCDE'));p.add_argument('probe',choices=['baseline','candidate','negative']);p.add_argument('label')
