...
import assess
from assess import ROOT,HERE,sha,verify_deployment

assert not (ROOT/'build/m13-exit-assessment').exists()
...
with tempfile.TemporaryDirectory(prefix='nc-m13-exit-smoke-') as temporary:
    target=pathlib.Path(temporary).resolve()
    assert target.parent==pathlib.Path(tempfile.gettempdir()).resolve() and target.name.startswith('nc-m13-exit-smoke-')
    assess.OUT=target
...
assert not target.exists() and not (ROOT/'build/m13-exit-assessment').exists()
...
data={'classification':'M13 NOT READY — ONE BOUNDED BLOCKER REMAINS','productionSourceDiffEmpty':True,'stagedDiffEmpty':True,'nativeAndManagedIntermediateObjectsRebuiltFromRestoredProductionSource':True,'deploymentUnchanged':True,'all49ShadersPerConfigurationUnchanged':True,'assetsUnchanged':True,'postCleanupSmoke':'PASS','frames':240,'temporaryLoaderEnvironmentRemoved':True,'scratchAbsent':True,'removedFiles':75,'launcher':str(launcher),'launcherSha256':sha(launcher),'head':final['head'],'originMain':final['originMain'],'main':subprocess.check_output(['git','rev-parse','main'],cwd=ROOT,text=True).strip(),'tagM13_4':subprocess.check_output(['git','rev-parse','m13.4-zero-contribution-terrain-material-noise^{}'],cwd=ROOT,text=True).strip(),'tagM13_3':final['m13.3-prepared-physical-terrain'],'branch':final['branch'],'gitDiffCheck':{'exitCode':check.returncode,'stdout':check.stdout,'stderr':check.stderr},'gitStatusShort':subprocess.check_output(['git','status','--short'],cwd=ROOT,text=True),'deployment':final['deployment'],'assets':final['assets']}
