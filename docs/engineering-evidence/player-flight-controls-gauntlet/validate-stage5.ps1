...
if($Resume){foreach($r in (Get-Content "$output/stage5-results.json" -Raw | ConvertFrom-Json)){if($r.exit -eq 0){$results.Add($r)}}}
function Gate([string]$name,[string]$exe,[string[]]$arguments) {
    ...
    $results | ConvertTo-Json -Depth 5 | Set-Content "$output/stage5-results.json"
...
foreach($configuration in @('debug','release')) {
    ...
    foreach($gate in @('pilot-allocation','pilot-demand','assembly-control','assembly-control-measure','assembly-production','srv01-integration','assembly-contact-cheap','assembly-powered-contact-cheap','assembly-departure-cheap','assembly-allocation')) {
        Gate "stage5-$gate-$configuration" $sim @("--$gate")
    }
    $graphics="$bin/NovaCore.Graphics.Tests/$configuration/NovaCore.Graphics.Tests.exe"
    foreach($gate in @('player-attitude','player-engine','assembly-presentation','florida-slab-camera-static','florida-slab-camera-warp','florida-slab-camera-moving','florida-slab-camera-costs','florida-slab-correctness','florida-slab-solar','florida-slab-allocation','assembly-florida-qualification')) {
        Gate "stage5-$gate-$configuration" $graphics @("--$gate")
    }
    Gate "stage5-layout-$configuration" $graphics @('--case=Transport layout')
    Gate "stage5-launcher-$configuration" "tests/NovaCore.Launcher.Tests/bin/$configuration/net10.0-windows/NovaCore.Launcher.Tests.exe" @()
    foreach($project in @('Simulation','ReferenceFrames','Precision','Camera','BepuDependency')) {
        Gate "stage5-$project-$configuration" "$bin/NovaCore.$project.Tests/$configuration/NovaCore.$project.Tests.exe" @()
    }
}
