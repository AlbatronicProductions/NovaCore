...
if($Resume){foreach($r in (Get-Content "$output/stage2-results.json" -Raw | ConvertFrom-Json)){if($r.exit -eq 0){$results.Add($r)}}}
function Gate([string]$name,[string]$exe,[string[]]$arguments) {
    ...
    $results | ConvertTo-Json -Depth 5 | Set-Content "$output/stage2-results.json"
...
foreach($configuration in @('debug','release')) {
    ...
    foreach($gate in @('assembly-control','assembly-control-measure','assembly-production','srv01-integration','assembly-contact-cheap','assembly-powered-contact-cheap','assembly-departure-cheap','assembly-allocation')) {
        Gate "stage2-$gate-$configuration" $sim @("--$gate")
    }
    ...
    foreach($gate in @('player-engine','assembly-presentation','florida-slab-camera-static','florida-slab-camera-warp','florida-slab-camera-moving','florida-slab-camera-costs','florida-slab-correctness','florida-slab-solar','florida-slab-allocation','assembly-florida-qualification')) {
        Gate "stage2-$gate-$configuration" $graphics @("--$gate")
    }
    Gate "stage2-layout-$configuration" $graphics @('--case=Transport layout')
    Gate "stage2-launcher-$configuration" "tests/NovaCore.Launcher.Tests/bin/$configuration/net10.0-windows/NovaCore.Launcher.Tests.exe" @()
    foreach($project in @('Simulation','ReferenceFrames','Precision','Camera','BepuDependency')) {
        Gate "stage2-$project-$configuration" "$bin/NovaCore.$project.Tests/$configuration/NovaCore.$project.Tests.exe" @()
    }
}
