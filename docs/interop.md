...
`NcFrameSubmission` also carries planetary, lighting, overlay, and generation-specific transport; the generic object layout above is not the complete frame ABI. All supported Earth routes submit NCSM1 topology/pupil and physical-preparation data to native-owned current/incoming resources, conservative culling/compaction, bounded VS/TCS/TES refinement, and indexed-indirect raster. Fence-confirmed publication controls generation ownership. The superseded anchored transport is retired; its 192-byte frame span at offset 576 is reserved and required to be zero, preserving subsequent fixed ABI offsets without an executable compatibility route. Global bootstrap remains only before complete NCSM1 readiness; independent validation/proof consumers remain development infrastructure. See [planetary-rendering.md](planetary-rendering.md) for the current contract.
